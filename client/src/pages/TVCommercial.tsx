...
// âââ Platform Presets âââ
const PLATFORMS = [
  { id: "broadcast-tv", label: "Broadcast TV", icon: Monitor, aspect: "16:9", description: "Network/cable television â NTSC/PAL broadcast standards", durations: [15, 30, 60], maxFileSize: "2GB" },
  { id: "streaming", label: "Streaming", icon: Tv, aspect: "16:9", description: "Netflix, Hulu, Disney+, Amazon â pre-roll and mid-roll", durations: [15, 30, 60, 90], maxFileSize: "4GB" },
  ...
  { id: "instagram", label: "Instagram", icon: ImageIcon, aspect: "1:1", description: "Feed, Stories, Reels â square and vertical formats", durations: [15, 30, 60], maxFileSize: "650MB" },
  ...
  { id: "facebook", label: "Facebook", icon: Globe, aspect: "16:9", description: "Feed ads, in-stream, Stories â auto-play optimized", durations: [15, 30, 60], maxFileSize: "4GB" },
  { id: "cinema", label: "Cinema Pre-Roll", icon: Film, aspect: "2.39:1", description: "Theater pre-show advertising â DCP format ready", durations: [30, 60, 90, 120], maxFileSize: "10GB" },
  { id: "connected-tv", label: "Connected TV (CTV)", icon: Tv, aspect: "16:9", description: "Roku, Apple TV, Fire TV â non-skippable premium", durations: [15, 30, 60], maxFileSize: "2GB" },
] as const;

type PlatformId = typeof PLATFORMS[number]["id"];

// âââ Commercial Formats âââ
const DURATIONS = [
  { seconds: 6, label: "6s Bumper", description: "YouTube bumper ad â single message, maximum impact" },
  { seconds: 15, label: "15s Spot", description: "Standard short spot â hook + sell + CTA" },
  { seconds: 30, label: "30s Spot", description: "Industry standard â full narrative arc with CTA" },
  { seconds: 60, label: "60s Spot", description: "Premium long-form â story-driven with emotional payoff" },
  { seconds: 90, label: "90s Extended", description: "Super Bowl / cinema â cinematic mini-film" },
  { seconds: 120, label: "2min Brand Film", description: "Brand storytelling â documentary or narrative style" },
];

// âââ Commercial Styles âââ
...
function getDefaultShots(durationSec: number): CommercialShot[] {
  ...
  if (durationSec <= 15) {
    return [
      { id: `shot-${Date.now()}-1`, label: "Hook", description: "Attention-grabbing opening â 2 seconds to capture", durationSec: 3, visualDescription: "", voiceoverText: "", onScreenText: "", musicMood: "impact", soundEffect: "", sceneId: null, transition: "cut" },
  ...
  return [
    ...
    { id: `shot-${Date.now()}-5`, label: "Rising Action", description: "Build momentum â montage of key moments", durationSec: 10, visualDescription: "", voiceoverText: "", onScreenText: "", musicMood: "driving", soundEffect: "", sceneId: null, transition: "whip" },
    { id: `shot-${Date.now()}-6`, label: "Climax", description: "The peak moment â maximum emotional impact", durationSec: 8, visualDescription: "", voiceoverText: "", onScreenText: "", musicMood: "epic", soundEffect: "bass-drop", sceneId: null, transition: "cut" },
    { id: `shot-${Date.now()}-7`, label: "Resolution", description: "Emotional payoff â the brand promise fulfilled", durationSec: 6, visualDescription: "", voiceoverText: "", onScreenText: "", musicMood: "hopeful", soundEffect: "", sceneId: null, transition: "dissolve" },
...
export default function TVCommercial() {
    ...
    onSuccess: (data: any) => {
      if (data?.suggestions) {
        setShots(prev => prev.map((s, i) => ({
          ...s,
          voiceoverText: data.suggestions[i] || s.voiceoverText || `[${s.label} â ${s.description}]`,
        })));
  ...
  const generateCommercialMutation = trpc.generation.generateTrailer.useMutation({
    onSuccess: () => {
      toast.success("Commercial generation started â check your project for the output");
    },
    onError: (err) => {
  ...
  // âââ Auto-assign scenes âââ
  ...
  const generateAIScript = () => {
    ...
    setTimeout(() => {
      ...
      toast.success("Voiceover script generated for each shot â refine the text as needed");
  ...
  return (
    <div className="min-h-screen text-foreground" style={{ background:"linear-gradient(135deg,#07070e 0%,#0c0b18 60%,#07070a 100%)" }}>
          ...
          <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] md:text-xs text-muted-foreground truncate">
                {project?.title || "Loading..."} â {currentPlatform?.label} Â· {duration}s Â· {shots.length} shots
              </p>
            </div>
          </div>
      ...
      <div className="flex h-[calc(100vh-57px)]">
        {/* âââ Left Panel: Platform & Format (desktop) âââ */}
        {!isMobile && (
        ...
        {/* âââ Center: Shot Storyboard âââ */}
        <div className="flex-1 overflow-y-auto p-2 md:p-4 min-w-0">
            ...
            <>
              ...
              {Math.abs(totalDuration - duration) > 2 && (
                <div className={`mb-3 p-2 rounded border text-xs ${totalDuration > duration ? "border-red-500/30 bg-red-500/5 text-red-400" : "border-amber-500/30 bg-amber-500/5 text-amber-400"}`}>
                  {totalDuration > duration
                    ? `Over target by ${totalDuration - duration}s â trim shots to fit ${duration}s format`
                    : `Under target by ${duration - totalDuration}s â add shots or extend durations`
                  }
                </div>
              )}
              ...
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold flex items-center gap-2 gradient-text-gold">
                  ...
                  Shots ({shots.length}) Â· {formatTime(totalDuration)}
                </h3>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-[9px] text-muted-foreground">â {shot.transition}</span>
          ...
          {tab === "script" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="text-xs text-muted-foreground">{wordCount} words Â· ~{estimatedReadTime}s read time / {duration}s target</div>
              </div>
          ...
          {tab === "brand" && (
            <div className="space-y-4">
              <h3 className="text-sm font-semibold gradient-text-gold">Brand Kit</h3>
        ...
        {/* âââ Right Panel: Shot Inspector (desktop) âââ */}
        {!isMobile && (
      ...
      {/* âââ Mobile Shot Inspector Sheet âââ */}
      {isMobile && (
