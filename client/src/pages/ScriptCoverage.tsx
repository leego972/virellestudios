import { useState } from "react";
  ...
  const SAMPLE_REPORT: CoverageReport = {
    ...
    format: "Feature",
    logline: "A disgraced detective must solve her daughter's disappearance in 48 hours â or become the prime suspect.",
    ...
    strengths: ["Compelling central protagonist with a clear internal flaw", "High commercial concept with existing audience appetite", "Strong dialogue â voice is consistent throughout", "Emotional stakes are clear from page one"],
    weaknesses: ["Second act loses momentum between pages 55â80", "Antagonist motivation is underwritten", "Resolution arrives slightly too easily â earned catharsis is missing", "Some expository scenes could be dramatised rather than stated"],
  ...
  export default function ScriptCoverage() {
    ...
    const analyzeMutation = trpc.coverage.analyze.useMutation({
      onSuccess: (data) => {
        ...
        toast.success("Coverage report generated â 5 credits deducted");
      },
      onError: (err) => {
    ...
    const analyze = () => {
      ...
      if (!apiKey.trim()) { toast.error("Add your OpenAI or Anthropic API key in Settings â API Keys, or paste it temporarily above"); return; }
  ...
  WEAKNESSES
  ${report.weaknesses.map(w => `â¢ ${w}`).join("\n")}
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">AI-powered script analysis â logline, scores, strengths, and a professional reader's report</p>
            </div>
          </div>
          <Badge variant="outline" className="flex items-center gap-1.5 text-xs">
            <Coins className="h-3 w-3 text-amber-500" />
            5 credits per analysis Â· {generationsLimit - generationsUsed} remaining
          </Badge>
        </div>
        ...
        {!report ? (
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="space-y-4">
              <Card>
                ...
                <CardContent className="space-y-4 glass-card">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5"><Label>Genre</Label><Input placeholder="Drama, Thrillerâ¦" value={genre} onChange={e => setGenre(e.target.value)} /></div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input type="password" placeholder="sk-â¦ or sk-ant-â¦" value={apiKey} onChange={e => setApiKey(e.target.value)} />
                    <p className="text-[10px] text-muted-foreground">Your key is sent to Virelle's server for this analysis only and never stored. Manage keys permanently in Settings â API Keys.</p>
                  </div>
                </CardContent>
              </Card>
              <div className="space-y-1.5">
              ...
              <div className="flex gap-3">
                <Button className="flex-1" onClick={analyze} disabled={analyzing}>
                  {analyzing ? <><Loader2 className="h-4 w-4 mr-2 animate-spin text-amber-400" />Analysingâ¦</> : <><Wand2 className="h-4 w-4 mr-2" />Generate Coverage Â· 5 credits</>}
                </Button>
            ...
            <Card className="flex flex-col justify-center items-center p-8 text-center border-dashed glass-card">
              ...
              <p className="text-xs text-muted-foreground mt-3 flex items-center gap-1"><Coins className="h-3 w-3 text-amber-500" />5 credits per analysis â uses your BYOK key</p>
            </Card>
          </div>
        ) : (
