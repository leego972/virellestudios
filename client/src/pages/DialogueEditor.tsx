        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-2 flex-wrap">
          ...
          <div className="flex gap-2 shrink-0">
            <Button variant="outline" size="sm" onClick={() => {
              if (conversationView.length === 0) { toast.error('No dialogue to export'); return; }
              const lines: string[] = [
                `DIALOGUE: ${project.data?.title || 'Untitled'}`,
                `Generated by VirElle Studios`,
                `Date: ${new Date().toLocaleDateString()}`,
                `Total Lines: ${conversationView.length}`,
                `${'='.repeat(60)}`,
                ``
              ];
              conversationView.forEach((d, idx) => {
                lines.push(`${d.characterName.toUpperCase()}${d.emotion && d.emotion !== 'neutral' ? ` (${d.emotion})` : ''}`);
                if (d.direction) lines.push(`  [${d.direction}]`);
                lines.push(`  ${d.line}`);
                lines.push(``);
              });
              ...
              a.href = url;
              a.download = `${(project.data?.title || 'dialogue').replace(/[^a-zA-Z0-9]/g, '_')}_dialogue.txt`;
              a.click();
                ...
                <ScrollArea className="h-[calc(100vh-16rem)]">
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 text-sm border-b border-border/30 hover:bg-accent/50 transition-colors ${
                      !selectedSceneId ? "bg-accent text-accent-amber-500" : ""
                    }`}
                  >
                    All Scenes
                  </button>
                  {sceneList.map((scene: any) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 text-sm border-b border-border/30 hover:bg-accent/50 transition-colors ${
                        selectedSceneId === scene.id ? "bg-accent text-accent-amber-500" : ""
                      }`}
                    >
