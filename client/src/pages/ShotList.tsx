    ...
    return (
      <div className="rounded-xl border p-5 space-y-5" style={{ borderColor: "rgba(212,175,55,0.25)", background: "rgba(212,175,55,0.03)" }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <Field label="Status"><Select value={s.status} onValueChange={v=>p({status:v as ShotStatus})}><SelectTrigger className="h-8 text-xs bg-black/30 border-border/40"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(STATUS_CONFIG).map(([k,v])=><SelectItem key={k} value={k} className="text-xs"><span className={v.color}>{v.label}</span></SelectItem>)}</SelectContent></Select></Field>
        </div>

        {/* Shot type and movement */}
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <Field label="Shot Type"><Select value={s.shotType} onValueChange={v=>p({shotType:v})}><SelectTrigger className="h-8 text-xs bg-black/30 border-border/40"><SelectValue /></SelectTrigger><SelectContent>{SHOT_TYPES.map(t=><SelectItem key={t} value={t} className="text-xs">{t}</SelectItem>)}</SelectContent></Select></Field>
          <Field label="Camera Movement"><Select value={s.cameraMovement} onValueChange={v=>p({cameraMovement:v})}><SelectTrigger className="h-8 text-xs bg-black/30 border-border/40"><SelectValue /></SelectTrigger><SelectContent>{MOVE_TYPES.map(m=><SelectItem key={m} value={m} className="text-xs">{m}</SelectItem>)}</SelectContent></Select></Field>
        ...
        <div className="rounded-xl border p-4 space-y-4" style={{ borderColor: "rgba(255,255,255,0.06)", background: "rgba(0,0,0,0.2)" }}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <Field label="Camera"><Select value={s.camera} onValueChange={v=>p({camera:v})}><SelectTrigger className="h-8 text-xs bg-black/30 border-border/40"><SelectValue /></SelectTrigger><SelectContent>{CAMERAS.map(c=><SelectItem key={c} value={c} className="text-xs">{c}</SelectItem>)}</SelectContent></Select></Field>
            <Field label="Lens"><Select value={s.lens} onValueChange={v=>p({lens:v})}><SelectTrigger className="h-8 text-xs bg-black/30 border-border/40"><SelectValue /></SelectTrigger><SelectContent>{LENSES.map(l=><SelectItem key={l} value={l} className="text-xs">{l}</SelectItem>)}</SelectContent></Select></Field>
            <Field label="Focal Length"><Select value={s.focalLength} onValueChange={v=>p({focalLength:v})}><SelectTrigger className="h-8 text-xs bg-black/30 border-border/40"><SelectValue /></SelectTrigger><SelectContent>{FOCAL_LENGTHS.map(f=><SelectItem key={f} value={f} className="text-xs">{f}</SelectItem>)}</SelectContent></Select></Field>
            <Field label="Aperture"><Select value={s.aperture} onValueChange={v=>p({aperture:v})}><SelectTrigger className="h-8 text-xs bg-black/30 border-border/40"><SelectValue /></SelectTrigger><SelectContent>{APERTURES.map(a=><SelectItem key={a} value={a} className="text-xs">{a}</SelectItem>)}</SelectContent></Select></Field>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <Field label="ND Filter"><Select value={s.ndFilter} onValueChange={v=>p({ndFilter:v})}><SelectTrigger className="h-8 text-xs bg-black/30 border-border/40"><SelectValue /></SelectTrigger><SelectContent>{ND_FILTERS.map(n=><SelectItem key={n} value={n} className="text-xs">{n}</SelectItem>)}</SelectContent></Select></Field>
            <Field label="Frame Rate"><Select value={s.frameRate} onValueChange={v=>p({frameRate:v})}><SelectTrigger className="h-8 text-xs bg-black/30 border-border/40"><SelectValue /></SelectTrigger><SelectContent>{FRAME_RATES.map(r=><SelectItem key={r} value={r} className="text-xs">{r}</SelectItem>)}</SelectContent></Select></Field>
            <Field label="Format / Codec"><Select value={s.format} onValueChange={v=>p({format:v})}><SelectTrigger className="h-8 text-xs bg-black/30 border-border/40"><SelectValue /></SelectTrigger><SelectContent>{FORMATS.map(f=><SelectItem key={f} value={f} className="text-xs">{f}</SelectItem>)}</SelectContent></Select></Field>
          </div>
        </div>
            ...
            <TabsContent value="list">
              ...
              <div className="flex items-center gap-3 flex-wrap mb-4">
                ...
                <Select value={filterType} onValueChange={setFilterType}>
                  <SelectTrigger className="h-8 text-xs w-40 bg-black/30 border-border/40"><SelectValue placeholder="Shot Type" /></SelectTrigger>
                ...
                <Select value={filterStatus} onValueChange={setFilterStatus}>
                  <SelectTrigger className="h-8 text-xs w-40 bg-black/30 border-border/40"><SelectValue placeholder="Status" /></SelectTrigger>
