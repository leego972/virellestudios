...
export default function PressKit() {
  ...
  return (
    ...
    <div className="container mx-auto p-4 max-w-6xl space-y-6">
      <SiteHead title={`Press Kit â ${project?.title || "Project"}`} description={`Electronic Press Kit for ${project?.title || "your film"} â synopsis, technical specs, festival selections and press quotes.`} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 print:grid-cols-1">
        <Card className="print:hidden glass-card shadow-lg shadow-amber-500/5">
          <CardHeader>
            <CardTitle className="gradient-text-gold glass-card shadow-lg shadow-amber-500/5">EPK Inputs</CardTitle>
          ...
          <CardContent className="space-y-3 glass-card shadow-lg shadow-amber-500/5">
            ...
            <div className="space-y-1.5"><Label>Festivals & Selections</Label><Textarea rows={2} value={kit.festivals} onChange={(e) => update("festivals", e.target.value)} placeholder="One per line: 2026 Tribeca Film Festival â Official Selection" /></div>
            ...
            <div className="space-y-1.5"><Label>Press Quotes</Label><Textarea rows={3} value={kit.pressQuotes} onChange={(e) => update("pressQuotes", e.target.value)} placeholder='"A genuine knockout." â IndieWire' /></div>
        ...
        <Card className="bg-white/5 border border-white/10 text-foreground print:bg-white print:text-black print:shadow-none print:border-none lg:col-span-1 print:col-span-1 glass-card shadow-lg shadow-amber-500/5">
          <CardContent className="p-8 prose max-w-none glass-card shadow-lg shadow-amber-500/5">
            ...
            {(characters?.length ?? 0) > 0 && (
              <section className="mb-4">
                ...
                <ul className="text-sm pl-5 list-disc">
                  {(characters ?? []).slice(0, 8).map((c: any) => (<li key={c.id}><strong>{c.name}</strong>{c.description ? ` â ${c.description}` : ""}</li>))}
                </ul>
              </section>
            )}
            {kit.directorBio && (
            ...
            <section className="border-t pt-3 mt-6 text-xs">
              <strong>{kit.productionCompany}</strong> {kit.contactEmail && <>Â· <a href={`mailto:${kit.contactEmail}`}>{kit.contactEmail}</a></>}
            </section>
          </CardContent>
        </Card>
      </div>
...
function buildMarkdown(title: string, kit: KitState, characters: any[]): string {
  const lines: string[] = [];
  lines.push(`# ${title} â Electronic Press Kit\n`);
  ...
  if (characters.length) {
    ...
    for (const c of characters.slice(0, 8)) lines.push(`- **${c.name}**${c.description ? ` â ${c.description}` : ""}`);
    lines.push("");
  }
  ...
  lines.push(`---\n**${kit.productionCompany}**${kit.contactEmail ? ` Â· ${kit.contactEmail}` : ""}`);
  return lines.join("\n");
}
