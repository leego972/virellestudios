  ...
  return (
    <div className="px-4 py-3 border-b bg-muted/20">
      <div className="flex items-center justify-between mb-2">
        ...
        {sent && <span className="text-[11px] text-green-500 font-medium">ÃÂ¢ÃÂÃÂ Sent to filmmaker</span>}
      </div>
      ...
      <textarea
        placeholder="Your note for the filmmaker ÃÂ¢ÃÂÃÂ pacing, performance, sound, storyÃÂ¢ÃÂÃÂ¦"
        value={comment}
      ...
      <div className="flex justify-end mt-2">
        ...
        >
          {add.isPending ? "SendingÃÂ¢ÃÂÃÂ¦" : "Send note"}
        </button>
      </div>
...
function ScreenerWatermark({ name }: { name: string }) {
  ...
  const label = `${name} ÃÂÃÂ· ${stamp} UTC ÃÂÃÂ· CONFIDENTIAL`;
  return (
 ...
 * No comments / approvals yet ÃÂ¢ÃÂÃÂ that's stage 2 of the review system.
 */
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <div className="mb-8">
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground mb-4">
            ...
            {project.mode && (<><span>ÃÂÃÂ·</span><span className="capitalize">{project.mode}</span></>)}
            {project.duration && (<><span>ÃÂÃÂ·</span><span>{project.duration}s</span></>)}
            {project.resolution && (<><span>ÃÂÃÂ·</span><span>{project.resolution}</span></>)}
          </div>
          {project.logline && (
