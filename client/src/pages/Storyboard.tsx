          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <Button variant="outline" size="sm" onClick={() => {
              ...
              scenes.forEach((scene, idx) => {
                ...
                if (scene.colorGrading) lines.push(`Color Grading: ${scene.colorGrading}`);
                lines.push(`Duration: ${formatTime(scene.duration || 30)}`);
                const charIds = scene.characterIds as number[] || [];
                if (charIds.length > 0) lines.push(`Cast: ${charIds.map(id => getCharName(id)).join(', ')}`);
                if (scene.productionNotes) lines.push(`Notes: ${scene.productionNotes}`);
                if (scene.transitionType) lines.push(`Transition: ${TRANSITION_LABELS[scene.transitionType] || scene.transitionType}`);
                lines.push(``);
              });
              ...
              a.href = url;
              a.download = `${project.title.replace(/[^a-zA-Z0-9]/g, '_')}_storyboard.txt`;
              a.click();
