import { useAuth } from "@/_core/hooks/useAuth";
  ...
  // âââ Scene Card (Grid) âââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
        ...
        <div className="p-3 flex-1 space-y-2">
          ...
          {(scene.characterIds as number[] || []).length > 0 && (
            <p className="text-[10px] text-muted-foreground/40 truncate">
              {(scene.characterIds as number[]).map((id: number) => getCharName(id)).join(" Â· ")}
            </p>
          )}
        </div>
      </div>
  ...
  // âââ Scene Row (List) âââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
        ...
        <div className="flex gap-4 p-3 rounded-xl border cursor-pointer transition-all hover:border-yellow-500/20"
          ...
          <div className="flex-1 min-w-0 space-y-1.5 py-0.5">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="text-sm font-semibold truncate">{scene.title || `Scene ${idx + 1}`}</h3>
  ...
  function StoryboardInner() {
    ...
    const exportTXT = () => {
      ...
      const lines = [
        ...
        `Generated by VirElle Studios â ${new Date().toLocaleDateString()}`,
        `Scenes: ${allScenes.length} Â· Duration: ${formatTime(totalDuration)}`,
        "=".repeat(60), "",
      ];
      allScenes.forEach((scene: any, idx: number) => {
        ...
        if (idx < allScenes.length - 1 && scene.transitionType) {
          lines.push(`â ${TRANSITION_LABELS[scene.transitionType] || scene.transitionType}`);
        }
        lines.push("");
      });
          ...
          <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="font-bold text-sm">{project.title} â Storyboard</div>
                  <div className="text-[10px] text-muted-foreground">{allScenes.length} scenes Â· {formatTime(totalDuration)} Â· {withVideo} with video</div>
                </div>
              </div>
            </div>
