...
export default function Projects() {
  ...
  const deleteMutation = trpc.project.delete.useMutation({
    ...
    onError: () => toast.error("Couldn't delete that project ÃÂ¢ÃÂÃÂ please try again, or refresh if it persists."),
  });

    const createDemoMut = trpc.project.createDemoShort.useMutation({
      onSuccess: (data) => {
        utils.project.list.invalidate();
        toast.success("Demo short generating ÃÂ¢ÃÂÃÂ 5 scenes firing now!");
  ...
  return (
    ...
    <div className="max-w-5xl mx-auto space-y-6 py-6 px-4">
      <SiteHead title="Projects" description="Manage all your AI film productions in one place ÃÂ¢ÃÂÃÂ scripts, scenes, casting, scoring, and distribution." />
        ...
        <Button size="sm" onClick={() => setLocation("/projects/new")} style={{background:"linear-gradient(135deg,#D4AF37,#b8960c)",color:"#000",fontWeight:600}}>
          ...
          >
            {createDemoMut.isPending
              ? <><Loader2 className="h-4 w-4 mr-1 animate-spin text-amber-400" />GeneratingÃÂ¢ÃÂÃÂ¦</>
      ...
      {isError ? (
        <Card className="bg-card/50 border-destructive/30 glass-card">
          <CardContent className="p-12 flex flex-col items-center text-center">
            ...
            <p className="text-xs text-muted-foreground mb-4">Network or server hiccup ÃÂ¢ÃÂÃÂ your work is safe. Retry below or refresh the page.</p>
        ...
        <CinematicEmptyState
          ...
          description={
            search || filterStatus !== "all"
              ...
              : "Every Virelle film starts as a project ÃÂ¢ÃÂÃÂ title, genre, characters, scenes. Open a fresh slate and start writing."
          }
          action={
            !search && filterStatus === "all" ? (
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                      ...
                      {(() => {
                        ...
                        return (
                          <Badge variant="outline" className="h-5 px-1.5 text-[10px] font-medium border-primary/30 bg-amber-400/5 text-amber-400">
                            Stage {stage}/8 ÃÂÃÂ· {meta.title}
                          </Badge>
                        );
                      })()}
                      <span className="text-border">ÃÂÃÂ·</span>
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      {project.createdAt && (
                        <>
                          <span className="text-border text-xs">ÃÂÃÂ·</span>
              ...
              <CardContent className="p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5 text-xs text-muted-foreground">
                    <span className="capitalize">{project.mode}</span>
                    {project.genre && <><span className="text-border">ÃÂÃÂ·</span><span>{project.genre}</span></>}
                    {project.createdAt && <><span className="text-border">ÃÂÃÂ·</span><span>{timeAgo(project.createdAt)}</span></>}
                  </div>
                </div>
                <Badge
                  variant="secondary"
