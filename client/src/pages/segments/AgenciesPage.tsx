/**
   * Creative Agencies — Virelle Studios segment landing page
   * Route: /agencies
  ...
  export default function CreativeAgenciesPage() {
    ...
    const handleDemo = () => {
      logEvent.mutate({ eventType: "demo_request", segment: SEGMENT, page: "/agencies", ...utm });
      // Primary: go to demo page or booking link
      const demoUrl = `/register?${new URLSearchParams({ utm_source: utm.source ?? "agencies", utm_medium: utm.utmMedium ?? "landing", utm_campaign: utm.utmCampaign ?? "agencies-demo", utm_content: "demo_cta" }).toString()}`;
    ...
    const handleEmailSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      if (!email) return;
      logEvent.mutate({ eventType: "email_capture", segment: SEGMENT, page: "/agencies", metadata: { email }, ...utm });
    ...
    const handleSignupClick = () => {
      logEvent.mutate({ eventType: "signup_click", segment: SEGMENT, page: "/agencies", ...utm });
      window.location.href = `/register?${new URLSearchParams({ utm_source: utm.source ?? "agencies", utm_medium: "landing", utm_campaign: "agencies-beta" }).toString()}`;
    };
    ...
    return (
      <div className="min-h-screen bg-neutral-950 text-white">
        ...
        <section className="px-6 py-24 max-w-5xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-bold leading-tight mb-6 whitespace-pre-line">Prototype campaign ideas
before production spend</h1>
          <p className="text-xl text-neutral-400 max-w-2xl mx-auto mb-10">Virelle Studios is the unfair advantage for creative agencies. Generate campaign videos, brand films, and ad concepts in hours — not weeks.</p>
          ...
          <div className="flex items-center justify-center gap-6 text-sm text-neutral-500 flex-wrap">
            ...
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-green-500" /> Pitch-ready in hours</span>
          </div>
        </section>

        {/* Capabilities */}
        <section className="px-6 py-16 max-w-5xl mx-auto">
          <h2 className="text-2xl font-bold text-center mb-12">Built for Creative Agencies</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 hover:border-neutral-700 transition-colors">
              ...
              <h3 className="font-semibold text-white mb-2">Campaign Prototype Videos</h3>
              <p className="text-sm text-neutral-400">Generate three campaign directions from one brief. Show clients exactly what each concept looks and feels like.</p>
            </div>
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 hover:border-neutral-700 transition-colors">
              <div className="w-10 h-10 rounded-lg bg-amber-900/40 flex items-center justify-center mb-4">
                <Zap className="w-5 h-5 text-amber-500" />
              </div>
              <h3 className="font-semibold text-white mb-2">Client Pitch Previews</h3>
              <p className="text-sm text-neutral-400">Present cinematic mockups before committing production budget. Win more pitches with less risk.</p>
            </div>
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 hover:border-neutral-700 transition-colors">
              <div className="w-10 h-10 rounded-lg bg-amber-900/40 flex items-center justify-center mb-4">
                <Star className="w-5 h-5 text-amber-500" />
              </div>
              <h3 className="font-semibold text-white mb-2">Multi-Format Asset Delivery</h3>
              <p className="text-sm text-neutral-400">Export social, broadcast, and digital formats simultaneously from a single source workflow.</p>
            </div>
          </div>
        </section>
        ...
        <section className="px-6 py-16 bg-neutral-900/50">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-5xl font-bold text-amber-500 my-6">10× faster pitches</p>
            <p className="text-lg text-neutral-400">Agencies using Virelle produce client pitch previews in hours instead of weeks — without production overhead.</p>
