import { useState, useEffect } from "react";
  ...
  export default function CreativeAgenciesPage() {
    ...
    useEffect(() => {
      logEvent.mutate({ eventType: "page_view", segment: SEGMENT, page: "/agencies", referrer: typeof document !== "undefined" ? document.referrer || undefined : undefined, ...utm });
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    const handleCta = () => {
      logEvent.mutate({ eventType: "cta_click", segment: SEGMENT, page: "/agencies", ...utm });
      window.location.href = `/register?${new URLSearchParams({ utm_source: utm.source ?? "agencies", utm_medium: utm.utmMedium ?? "landing", utm_campaign: utm.utmCampaign ?? "agencies-beta", utm_content: "hero_cta" }).toString()}`;
    };

    const handleEmailSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      if (!email) return;
      logEvent.mutate({ eventType: "signup", segment: SEGMENT, page: "/agencies", metadata: { email }, ...utm });
      setSubmitted(true);
...
before production spend</h1>
        ...
        <section className="px-6 py-16 max-w-5xl mx-auto">
          <h2 className="text-2xl font-bold text-center mb-12 text-neutral-100">Built for Creative Agencies</h2>
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 hover:border-neutral-700 transition-colors">
              ...
              <p className="text-sm text-neutral-400">Export social, broadcast, and digital OOH formats simultaneously from a single source workflow.</p>
            </div>
          </div>
        </section>
