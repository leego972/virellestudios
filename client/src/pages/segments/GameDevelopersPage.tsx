/**
   * Game Developers — Virelle Studios segment landing page
   * Route: /game-trailers
  ...
  export default function GameDevelopersPage() {
    ...
    const handleDemo = () => {
      logEvent.mutate({ eventType: "demo_request", segment: SEGMENT, page: "/game-trailers", ...utm });
      // Primary: go to demo page or booking link
      const demoUrl = `/register?${new URLSearchParams({ utm_source: utm.source ?? "game-trailers", utm_medium: utm.utmMedium ?? "landing", utm_campaign: utm.utmCampaign ?? "game-trailers-demo", utm_content: "demo_cta" }).toString()}`;
    ...
    const handleEmailSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      if (!email) return;
      logEvent.mutate({ eventType: "email_capture", segment: SEGMENT, page: "/game-trailers", metadata: { email }, ...utm });
    ...
    const handleSignupClick = () => {
      logEvent.mutate({ eventType: "signup_click", segment: SEGMENT, page: "/game-trailers", ...utm });
      window.location.href = `/register?${new URLSearchParams({ utm_source: utm.source ?? "game-trailers", utm_medium: "landing", utm_campaign: "game-trailers-beta" }).toString()}`;
    };
    ...
    return (
      <div className="min-h-screen bg-neutral-950 text-white">
        ...
        <section className="px-6 py-24 max-w-5xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-bold leading-tight mb-6 whitespace-pre-line">Cinematic game trailers
that sell your game</h1>
          <p className="text-xl text-neutral-400 max-w-2xl mx-auto mb-10">Virelle Studios helps indie game teams produce hype trailers, announcement teasers, and publisher pitch videos — without a cinematics budget.</p>
          ...
          <div className="flex items-center justify-center gap-6 text-sm text-neutral-500 flex-wrap">
            ...
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-green-500" /> Steam-ready trailer concepts</span>
          </div>
        </section>

        {/* Capabilities */}
        <section className="px-6 py-16 max-w-5xl mx-auto">
          <h2 className="text-2xl font-bold text-center mb-12">Built for Game Developers</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 hover:border-neutral-700 transition-colors">
              ...
              <h3 className="font-semibold text-white mb-2">Launch Trailer Concepts</h3>
              <p className="text-sm text-neutral-400">Generate cinematic launch trailer concepts using your game's art style, lore, and story beats.</p>
            </div>
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 hover:border-neutral-700 transition-colors">
              <div className="w-10 h-10 rounded-lg bg-amber-900/40 flex items-center justify-center mb-4">
                <Zap className="w-5 h-5 text-amber-500" />
              </div>
              <h3 className="font-semibold text-white mb-2">Publisher Pitch Materials</h3>
              <p className="text-sm text-neutral-400">Combine your gameplay brief with AI-generated pitch visuals for investor and publisher meetings.</p>
            </div>
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 hover:border-neutral-700 transition-colors">
              <div className="w-10 h-10 rounded-lg bg-amber-900/40 flex items-center justify-center mb-4">
                <Star className="w-5 h-5 text-amber-500" />
              </div>
              <h3 className="font-semibold text-white mb-2">Announcement Teasers</h3>
              <p className="text-sm text-neutral-400">Create mystery announcement teasers that drive Steam wishlists and community hype.</p>
            </div>
          </div>
        </section>
        ...
        <section className="px-6 py-16 bg-neutral-900/50">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-5xl font-bold text-amber-500 my-6">$15,000–$40,000 saved</p>
            <p className="text-lg text-neutral-400">vs. outsourcing a cinematic trailer concept to a specialist game trailer studio.</p>
