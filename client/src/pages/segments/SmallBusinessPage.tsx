/**
   * Small Businesses — Virelle Studios segment landing page
   * Route: /small-business-video
  ...
  export default function SmallBusinessesPage() {
    ...
    const handleDemo = () => {
      logEvent.mutate({ eventType: "demo_request", segment: SEGMENT, page: "/small-business-video", ...utm });
      // Primary: go to demo page or booking link
      const demoUrl = `/register?${new URLSearchParams({ utm_source: utm.source ?? "small-business-video", utm_medium: utm.utmMedium ?? "landing", utm_campaign: utm.utmCampaign ?? "small-business-video-demo", utm_content: "demo_cta" }).toString()}`;
    ...
    const handleEmailSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      if (!email) return;
      logEvent.mutate({ eventType: "email_capture", segment: SEGMENT, page: "/small-business-video", metadata: { email }, ...utm });
    ...
    const handleSignupClick = () => {
      logEvent.mutate({ eventType: "signup_click", segment: SEGMENT, page: "/small-business-video", ...utm });
      window.location.href = `/register?${new URLSearchParams({ utm_source: utm.source ?? "small-business-video", utm_medium: "landing", utm_campaign: "small-business-video-beta" }).toString()}`;
    };
    ...
    return (
      <div className="min-h-screen bg-neutral-950 text-white">
        ...
        <section className="px-6 py-24 max-w-5xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-bold leading-tight mb-6 whitespace-pre-line">Cinematic video ads
without the agency price tag</h1>
          <p className="text-xl text-neutral-400 max-w-2xl mx-auto mb-10">Virelle Studios gives small businesses the same video production capability as enterprise brands — at a fraction of the concepting cost.</p>
          ...
          <div className="flex items-center justify-center gap-6 text-sm text-neutral-500 flex-wrap">
            ...
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-green-500" /> No film crew needed</span>
          </div>
        </section>

        {/* Capabilities */}
        <section className="px-6 py-16 max-w-5xl mx-auto">
          <h2 className="text-2xl font-bold text-center mb-12">Built for Small Businesses</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 hover:border-neutral-700 transition-colors">
              ...
              <h3 className="font-semibold text-white mb-2">Product & Service Ads</h3>
              <p className="text-sm text-neutral-400">Create scroll-stopping social ads for Instagram, TikTok, and YouTube — in minutes, not days.</p>
            </div>
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 hover:border-neutral-700 transition-colors">
              <div className="w-10 h-10 rounded-lg bg-amber-900/40 flex items-center justify-center mb-4">
                <Zap className="w-5 h-5 text-amber-500" />
              </div>
              <h3 className="font-semibold text-white mb-2">Brand Film Concepts</h3>
              <p className="text-sm text-neutral-400">Tell your brand story with a cinematic brand film concept before committing to a full production.</p>
            </div>
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 hover:border-neutral-700 transition-colors">
              <div className="w-10 h-10 rounded-lg bg-amber-900/40 flex items-center justify-center mb-4">
                <Star className="w-5 h-5 text-amber-500" />
              </div>
              <h3 className="font-semibold text-white mb-2">Seasonal Campaigns</h3>
              <p className="text-sm text-neutral-400">Generate fresh campaign visual concepts for every season and promotion without an agency retainer.</p>
            </div>
          </div>
        </section>
        ...
        <section className="px-6 py-16 bg-neutral-900/50">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-5xl font-bold text-amber-500 my-6">60–90% less on concepting</p>
            <p className="text-lg text-neutral-400">Virelle helps small businesses validate video concepts before committing to production spend.</p>
