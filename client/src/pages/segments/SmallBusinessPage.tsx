import { useState, useEffect } from "react";
  ...
  export default function SmallBusinessesPage() {
    ...
    useEffect(() => {
      logEvent.mutate({ eventType: "page_view", segment: SEGMENT, page: "/small-business-video", referrer: typeof document !== "undefined" ? document.referrer || undefined : undefined, ...utm });
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    const handleCta = () => {
      logEvent.mutate({ eventType: "cta_click", segment: SEGMENT, page: "/small-business-video", ...utm });
      window.location.href = `/register?${new URLSearchParams({ utm_source: utm.source ?? "small-business-video", utm_medium: utm.utmMedium ?? "landing", utm_campaign: utm.utmCampaign ?? "small-business-video-beta", utm_content: "hero_cta" }).toString()}`;
    };

    const handleEmailSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      if (!email) return;
      logEvent.mutate({ eventType: "signup", segment: SEGMENT, page: "/small-business-video", metadata: { email }, ...utm });
      setSubmitted(true);
...
without the agency price tag</h1>
        ...
        <section className="px-6 py-16 max-w-5xl mx-auto">
          <h2 className="text-2xl font-bold text-center mb-12 text-neutral-100">Built for Small Businesses</h2>
