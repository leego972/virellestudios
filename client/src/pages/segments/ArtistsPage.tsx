import { useState, useEffect } from "react";
  ...
  export default function MusicArtistsVisualArtistsPage() {
    ...
    useEffect(() => {
      logEvent.mutate({ eventType: "page_view", segment: SEGMENT, page: "/artists", referrer: typeof document !== "undefined" ? document.referrer || undefined : undefined, ...utm });
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    const handleCta = () => {
      logEvent.mutate({ eventType: "cta_click", segment: SEGMENT, page: "/artists", ...utm });
      window.location.href = `/register?${new URLSearchParams({ utm_source: utm.source ?? "artists", utm_medium: utm.utmMedium ?? "landing", utm_campaign: utm.utmCampaign ?? "artists-beta", utm_content: "hero_cta" }).toString()}`;
    };
...
cinematic films — instantly</h1>
        ...
        <section className="px-6 py-16 max-w-5xl mx-auto">
          <h2 className="text-2xl font-bold text-center mb-12 text-neutral-100">Built for Music Artists & Visual Artists</h2>
