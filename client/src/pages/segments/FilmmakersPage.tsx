/**
   * Indie Filmmakers — Virelle Studios segment landing page
   * Route: /filmmakers
  ...
  export default function IndieFilmmakersPage() {
    const utm = useUtm();
    ...
    const handleDemo = () => {
      logEvent.mutate({ eventType: "demo_request", segment: SEGMENT, page: "/filmmakers", ...utm });
      // Primary: go to demo page or booking link
      const demoUrl = `/register?${new URLSearchParams({ utm_source: utm.source ?? "filmmakers", utm_medium: utm.utmMedium ?? "landing", utm_campaign: utm.utmCampaign ?? "filmmakers-demo", utm_content: "demo_cta" }).toString()}`;
    ...
    const handleEmailSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      if (!email) return;
      logEvent.mutate({ eventType: "email_capture", segment: SEGMENT, page: "/filmmakers", metadata: { email }, ...utm });
    ...
    const handleSignupClick = () => {
      logEvent.mutate({ eventType: "signup_click", segment: SEGMENT, page: "/filmmakers", ...utm });
      window.location.href = `/register?${new URLSearchParams({ utm_source: utm.source ?? "filmmakers", utm_medium: "landing", utm_campaign: "filmmakers-beta" }).toString()}`;
    };
    ...
    return (
      <div className="min-h-screen bg-neutral-950 text-white">
        ...
        <section className="px-6 py-24 max-w-5xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-bold leading-tight mb-6 whitespace-pre-line">Pitch your film visually
before you shoot it</h1>
          <p className="text-xl text-neutral-400 max-w-2xl mx-auto mb-10">From screenplay to storyboard, trailer, and investor pitch. Virelle gives independent filmmakers a full pre-production studio — without the traditional budget.</p>
          ...
          <div className="flex items-center justify-center gap-6 text-sm text-neutral-500 flex-wrap">
            ...
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-green-500" /> Script to visual in minutes</span>
          </div>
        </section>

        {/* Capabilities */}
        <section className="px-6 py-16 max-w-5xl mx-auto">
          <h2 className="text-2xl font-bold text-center mb-12">Built for Indie Filmmakers</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 hover:border-neutral-700 transition-colors">
              ...
              <h3 className="font-semibold text-white mb-2">Proof-of-Concept Trailers</h3>
              <p className="text-sm text-neutral-400">Generate cinematic proof-of-concept trailers that show investors exactly what your film will look like.</p>
            </div>
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 hover:border-neutral-700 transition-colors">
              <div className="w-10 h-10 rounded-lg bg-amber-900/40 flex items-center justify-center mb-4">
              ...
              <p className="text-sm text-neutral-400">Visualise every scene from your script in seconds. No illustrator needed for the first creative pass.</p>
            </div>
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 hover:border-neutral-700 transition-colors">
              <div className="w-10 h-10 rounded-lg bg-amber-900/40 flex items-center justify-center mb-4">
              ...
              <p className="text-sm text-neutral-400">Auto-generate investor-ready pitch decks with your characters, scenes, and director vision document.</p>
            </div>
          </div>
        </section>
        ...
        <section className="px-6 py-16 bg-neutral-900/50">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-5xl font-bold text-amber-500 my-6">$25,000+ saved</p>
            <p className="text-lg text-neutral-400">vs. traditional pre-production costs for storyboarding, VFX tests, and pitch material on an indie feature.</p>
