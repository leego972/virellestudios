import { useState } from "react";
  ...
  const INITIAL_POSTS = [
    ...
    { id: 6, author: "Diego F.", avatar: "DF", role: "Editor", title: "Request: feedback on my short film trailer cut (2 min)", body: "Finished my first narrative short ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ a 12-minute thriller. Would love eyes on the trailer cut before I submit to festivals. Link in comments.", category: "Feedback", likes: 12, replies: 9, time: "3d ago", pinned: false },
  ];

  const EVENTS = [
    { title: "Sundance Film Festival", date: "Jan 23ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂFeb 2, 2026", location: "Park City, Utah", type: "Festival", url: "https://sundance.org" },
    { title: "Berlin International Film Festival", date: "Feb 13ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ23, 2026", location: "Berlin, Germany", type: "Festival", url: "https://berlinale.de" },
    { title: "Hot Docs International Documentary Festival", date: "May 1ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ11, 2026", location: "Toronto, Canada", type: "Festival", url: "https://hotdocs.ca" },
    { title: "Cannes Film Market (MarchÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ© du Film)", date: "May 13ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ24, 2026", location: "Cannes, France", type: "Market", url: "https://marchedufilm.com" },
    { title: "AFM ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ American Film Market", date: "Nov 5ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ10, 2025", location: "Las Vegas, NV", type: "Market", url: "https://americanfilmmarket.com" },
    { title: "SXSW Film & TV Festival", date: "Mar 6ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ15, 2026", location: "Austin, Texas", type: "Festival", url: "https://sxsw.com/film" },
  ];
  ...
  const CATEGORY_COLORS: Record<string, string> = {
    ...
    Finance: "bg-green-500/15 text-green-500", Music: "bg-pink-500/15 text-pink-500", Feedback: "bg-amber-500/15 text-amber-400",
    ...
    return (
      ...
      <div className="max-w-5xl mx-auto space-y-6">
          ...
          <DialogContent className="sm:max-w-lg">
            ...
            <DialogFooter>
              ...
              <Button onClick={handleNewPost} disabled={submitting}>
                {submitting ? "PostingÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¦" : "Post to Community"}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Tabs defaultValue="discussions">
                  ...
                  <CardContent className="p-4 glass-card shadow-lg shadow-amber-500/5 hover:shadow-amber-500/20 transition-shadow">
                    <div className="flex items-start gap-3">
                      <Avatar className="h-9 w-9 shrink-0 mt-0.5">
                        <AvatarFallback className="text-xs font-bold bg-amber-400/10 text-amber-400">{post.avatar}</AvatarFallback>
                      </Avatar>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          {post.pinned && <Badge className="text-[10px] h-4">ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Pinned</Badge>}
                        ...
                        <div className="flex items-center gap-4 mt-2">
                          <span className="text-xs text-muted-foreground">{post.author} ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ· {post.role} ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ· {post.time}</span>
          ...
          {/* ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Events ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ */}
          <TabsContent value="events" className="space-y-3 mt-4">
            <p className="text-sm text-muted-foreground">Upcoming industry events, film markets, and festivals.</p>
            {EVENTS.map(ev => (
              <Card key={ev.title} className="hover:border-primary/30 transition-colors glass-card shadow-lg shadow-amber-500/5 hover:shadow-amber-500/20">
                <CardContent className="p-4 flex items-center gap-4 glass-card shadow-lg shadow-amber-500/5 hover:shadow-amber-500/20 transition-shadow">
          ...
          {/* ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Resources Directory ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ */}
          <TabsContent value="resources" className="space-y-4 mt-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {filteredResources.map(r => (
                <Card key={r.title} className="hover:border-primary/40 transition-colors glass-card shadow-lg shadow-amber-500/5 hover:shadow-amber-500/20">
                  <CardContent className="p-3 space-y-1.5 glass-card shadow-lg shadow-amber-500/5 hover:shadow-amber-500/20 transition-shadow">
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-semibold text-sm">{r.title}</span>
                      <Badge variant="outline" className="text-[10px] shrink-0">{r.category}</Badge>
                    </div>
                    <p className="text-xs text-muted-foreground">{r.description}</p>
                    <a href={r.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-amber-400 hover:underline"><ExternalLink className="h-3 w-3" />{r.url.replace("https://","")}</a>
                  </CardContent>
                </Card>
              ))}
            </div>
