...
function VFXSuiteInner() {
  ...
  const selectedSwappys = selectedOps.some((operation) => SWAPPYS_OPS.has(operation)) || transformGoal !== "appearance_reference";
  ...
  const estimatedCredits = selectedSwappys
    ? exportQuality === "preview"
      ? 12 + Math.ceil(mediaCount / 3)
      : exportQuality === "final"
        ? 24 + Math.ceil(mediaCount / 2)
        : 40 + mediaCount
    : selectedOps.length <= 2
      ? 6
      : selectedOps.length <= 5
        ? 12
        : 18;
  ...
  const handleProcess = async () => {
    if (selectedOps.length === 0 && transformGoal === "appearance_reference") {
      return toast.error("Select a VFX or Swappys transform operation.");
    }
      ...
      <div className="sticky top-0 z-20 border-b px-4 py-3" style={{ borderColor: "rgba(255,255,255,0.07)", background: "rgba(7,7,14,0.97)", backdropFilter: "blur(24px)" }}>
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" className="bg-amber-500 text-black hover:bg-amber-600" onClick={handleProcess} disabled={isProcessing}>
              {isProcessing ? <><Loader2 className="mr-1 h-4 w-4 animate-spin" />Processing</> : processComplete ? <><CheckCircle2 className="mr-1 h-4 w-4" />Created</> : <><Wand2 className="mr-1 h-4 w-4" />Create Job</>}
            </Button>
          </div>
        </div>
      </div>
          ...
          <Card className="border-amber-500/20 glass-card">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                ...
                <div><Label className="text-xs text-muted-foreground">Presentation / Style</Label><input value={targetPresentation} onChange={(event) => setTargetPresentation(event.target.value)} placeholder="cinematic, tasteful, masculine…" className="mt-1 w-full rounded-md border border-border bg-black/30 px-3 py-2 text-sm" /></div>
              </div>
