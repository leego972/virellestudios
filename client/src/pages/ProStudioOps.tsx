...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Live Presence Bar (heartbeats every 12s, polls every 6s) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
    ...
    <div className="flex items-center gap-2 text-xs text-muted-foreground">
      ...
      <div className="flex -space-x-1">
        {users.slice(0, 6).map((u: any, i: number) => {
          ...
          return (
            <div key={i} title={`${u.email || "user"} ÃÂÃÂÃÂÃÂ· ${u.tab || ""}`} className="h-6 w-6 rounded-full bg-emerald-600/80 text-white text-[10px] flex items-center justify-center border border-background ring-1 ring-emerald-400/30">
              {initial}
            </div>
          );
        })}
...
export default function ProStudioOps() {
  ...
  // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Pro NLE keyboard shortcuts (only when not typing in form fields) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      else if (e.key === "?") { toast.info("Shortcuts: 1-0 = tabs ÃÂÃÂÃÂÃÂ· D Dashboard ÃÂÃÂÃÂÃÂ· F Frames ÃÂÃÂÃÂÃÂ· Q Queue ÃÂÃÂÃÂÃÂ· P Deliverables ÃÂÃÂÃÂÃÂ· L Locks ÃÂÃÂÃÂÃÂ· A Approvals ÃÂÃÂÃÂÃÂ· B Budget ÃÂÃÂÃÂÃÂ· X Cuts"); e.preventDefault(); }
    };
    ...
    <div className="container mx-auto px-4 py-6 max-w-7xl overflow-x-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground">Pro production workflow ÃÂÃÂÃÂÃÂ· live multi-user collab ÃÂÃÂÃÂÃÂ· server-enforced render budget ÃÂÃÂÃÂÃÂ· NLE shortcuts (press ?)</p>
        </div>
...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Studio Dashboard: single pane of glass for production readiness ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
function StudioDashboardTab({ projectId }: { projectId: number }) {
  ...
  if (!d) return <div className="p-6 text-sm text-muted-foreground">Loading studio metricsÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦</div>;
    ...
    <div className="max-w-7xl mx-auto space-y-4 px-4 py-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Card><CardContent className="pt-4 glass-card shadow-lg shadow-amber-500/5">
          ...
          <div className="text-[11px] text-muted-foreground mt-1">
            <span className="text-blue-400">{d.queue.running} running</span> ÃÂÃÂÃÂÃÂ· <span>{d.queue.queued} queued</span>
            {d.queue.failed > 0 && <span className="text-rose-400"> ÃÂÃÂÃÂÃÂ· {d.queue.failed} failed</span>}
            {d.queue.paused > 0 && <span className="text-amber-400"> ÃÂÃÂÃÂÃÂ· {d.queue.paused} paused</span>}
          </div>
        </CardContent></Card>
        <Card><CardContent className="pt-4 glass-card shadow-lg shadow-amber-500/5">
          ...
          <CardContent className="pt-5 pb-5 glass-card shadow-lg shadow-amber-500/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <div className="text-[11px] text-muted-foreground mt-0.5">vs ${d.savings.tradEquivalentUsd.toLocaleString()} traditional ÃÂÃÂÃÂÃÂ· spent ${d.savings.spentUsd.toLocaleString()}</div>
              </div>
              <div>
                ...
                <div className="text-[11px] text-muted-foreground mt-0.5">{d.savings.renderedScenes} scenes rendered ÃÂÃÂÃÂÃÂ· ~1 shoot day each</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground flex items-center gap-1.5"><Sparkles className="h-3 w-3" />Cost multiplier</div>
                <div className="text-3xl font-bold text-violet-300 gradient-text-gold">{d.savings.savingsMultiplier ? `${d.savings.savingsMultiplier}ÃÂÃÂÃÂÃÂ` : "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ"}</div>
        ...
        <Card className="border-violet-500/30 bg-violet-500/5 glass-card shadow-lg shadow-amber-500/5"><CardContent className="pt-4 flex flex-wrap items-center justify-between gap-3 glass-card shadow-lg shadow-amber-500/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-base font-semibold">{d.forecast.unrenderedScenes} scenes left to render ÃÂÃÂÃÂÃÂ· est <span className="text-violet-300">{d.forecast.estimatedCredits} credits</span></div>
            </div>
          </div>
          <div className="text-[11px] text-muted-foreground">
            {d.spend.dailyCap != null ? `ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${Math.ceil(d.forecast.estimatedCredits / d.spend.dailyCap)} day(s) at current daily cap` : "No daily cap set"}
            {d.scenes.locked > 0 && <span className="ml-3 text-amber-400">ÃÂÃÂÃÂÃÂ· ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ {d.scenes.locked} locked scene(s)</span>}
          </div>
        </CardContent></Card>
      )}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <Card><CardHeader><CardTitle className="text-sm flex items-center gap-2 gradient-text-gold glass-card shadow-lg shadow-amber-500/5"><MessageSquare className="h-4 w-4" />Frame Reviews</CardTitle></CardHeader><CardContent>
          <div className="flex items-baseline gap-3"><div className="text-2xl font-bold">{d.comments.open}</div><div className="text-xs text-muted-foreground">open ÃÂÃÂÃÂÃÂ· {d.comments.resolved} resolved</div></div>
          {d.comments.open > 0 && <div className="text-[11px] text-amber-400 mt-1">ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ  awaiting director attention</div>}
        </CardContent></Card>
        ...
        <Card><CardHeader><CardTitle className="text-sm flex items-center gap-2 gradient-text-gold glass-card shadow-lg shadow-amber-500/5"><Scale className="h-4 w-4" />Clearances</CardTitle></CardHeader><CardContent>
          ...
          {d.clearances.pending > 0 && <div className="text-[11px] text-rose-400 mt-1">ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ  {d.clearances.pending} blocking distribution</div>}
        </CardContent></Card>
      </div>
      {d.queue.cap?.pauseOnExceed && (
        <Card className="border-emerald-500/30 bg-emerald-500/5 glass-card shadow-lg shadow-amber-500/5"><CardContent className="pt-4 glass-card shadow-lg shadow-amber-500/5">
          ...
          <div className="text-[11px] text-muted-foreground mt-1">Server-side enforcement: per-job cap {d.queue.cap.perJobCredits ?? "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ"}cr ÃÂÃÂÃÂÃÂ· daily cap {d.queue.cap.dailyCredits ?? "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ"}cr ÃÂÃÂÃÂÃÂ· jobs over cap will be rejected at the generation chokepoint.</div>
        </CardContent></Card>
      )}
    </div>
  );
...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Frame Comments ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
        ...
        <CardContent className="p-2 max-h-[60vh] overflow-y-auto glass-card shadow-lg shadow-amber-500/5">
          {sceneList.map(s => (
            <button key={s.id} onClick={() => { setSel(s.id); setDraft(null); }} className={`w-full text-left p-2 rounded text-xs hover:bg-muted/50 ${sceneId === s.id ? "bg-muted/70 ring-1 ring-primary/30" : ""}`}>
              ...
              <div className="text-[10px] text-muted-foreground">{(map.get(s.id) || []).length} comments ÃÂÃÂÃÂÃÂ· {(map.get(s.id) || []).filter(c => c.status === "open").length} open</div>
            </button>
          ))}
        </CardContent>
      </Card>
...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Color Pipeline ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
      ...
      <Card>
        ...
        <CardContent className="space-y-3 glass-card shadow-lg shadow-amber-500/5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><Label className="text-xs">LUT Name</Label><Input value={live.lutName || ""} onChange={e => setDraft({ ...live, lutName: e.target.value })} placeholder="Arri LogC ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Rec709" className="h-8 text-xs" /></div>
...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Asset Versions ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <Card><CardHeader><CardTitle className="text-sm gradient-text-gold glass-card shadow-lg shadow-amber-500/5">New Snapshot</CardTitle><p className="text-xs text-muted-foreground">Pin a version of any project asset for rollback or audit.</p></CardHeader>
        <CardContent className="space-y-2 glass-card shadow-lg shadow-amber-500/5">
          ...
          <div><Label className="text-xs">Version Label</Label><Input value={form.label} onChange={e => setForm({ ...form, label: e.target.value })} placeholder="v3 ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ investor draft" className="h-8 text-xs" /></div>
...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Render Queue ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
function RenderQueueTab({ projectId }: { projectId: number }) {
  ...
  const runVid = trpc.scene.generateVideo.useMutation({
    onSuccess: () => { toast.success("Render kicked off ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ credits deducted, video generation in progress."); q.refetch(); },
    onError: (err: any) => { toast.error(err?.message || "Render failed"); q.refetch(); },
  });
  const runNow = (job: any) => {
    if (!job.sceneId) { toast.error("This job has no scene attached ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ set sceneId first."); return; }
  ...
  const persist = () => { save.mutate({ projectId, data: live }); audit.mutate({ projectId, event: { action: "renderQueue.save", summary: `Queue: ${live.jobs.length} jobs, cap=${live.cap?.dailyCredits || "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ"}/day` } }); setDraft(null); };
  ...
  return (
    <div className="space-y-4">
      ...
      <Card><CardHeader className="flex flex-row items-center justify-between flex-wrap gap-2 glass-card shadow-lg shadow-amber-500/5"><CardTitle className="text-sm gradient-text-gold glass-card shadow-lg shadow-amber-500/5">Queue ({live.jobs.length} jobs ÃÂÃÂÃÂÃÂ· est {dailyEstimate} cr {overCap && <span className="text-rose-400">ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ  over cap</span>})</CardTitle>
        ...
        <CardContent className="space-y-2 glass-card shadow-lg shadow-amber-500/5">
          {live.jobs.map((j: any, i: number) => (
            <div key={j.id} className="border rounded p-2 grid grid-cols-12 gap-2 min-w-[600px] items-center text-xs">
              ...
              <Select value={j.sceneId == null ? "none" : String(j.sceneId)} onValueChange={v => updateJob(i, { sceneId: v === "none" ? null : Number(v) })}><SelectTrigger className="h-8 text-xs col-span-2"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none" className="text-xs">ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ no scene ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ</SelectItem>{(scenes.data || []).map((s: any) => <SelectItem key={s.id} value={String(s.id)} className="text-xs">{s.title || `Scene ${s.id}`}</SelectItem>)}</SelectContent></Select>
              ...
              <div className="flex gap-0.5 col-span-1 justify-end">
                ...
                <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => moveUp(i)} aria-label="Move job up in queue"><span aria-hidden="true">ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ</span></Button>
...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Deliverables ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Clearances ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Distribution ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Audit Log ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Proxy Chain ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
function ProxyTab({ projectId }: { projectId: number }) {
  ...
  return (
    <Card><CardHeader><CardTitle className="text-sm gradient-text-gold glass-card shadow-lg shadow-amber-500/5">Proxy ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Master Conform</CardTitle><p className="text-xs text-muted-foreground">Edit on 1/4-res proxies, conform to full-res master only on final pass.</p></CardHeader>
...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Cuts & Transitions ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Scene Locks: prevent accidental re-renders of approved scenes ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
function LocksTab({ projectId }: { projectId: number }) {
  ...
  const submitLockDialog = () => {
    ...
    audit.mutate({ projectId, event: { action: "sceneLock.lock", targetType: "scene", targetId: String(lockDialog.sceneId), summary: `Locked: ${lockDialog.label} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${reason}` } });
    setLockDialog(null);
  };
  ...
  if (!scenes.data?.length) return <Card><CardContent className="pt-6 text-sm text-muted-foreground glass-card shadow-lg shadow-amber-500/5">No scenes yet ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ create scenes first to lock approved renders.</CardContent></Card>;
              ...
              <div className="flex-1 min-w-0">
                ...
                {isLocked && lk && (
                  <div className="text-[11px] text-muted-foreground mt-0.5 ml-5">
                    ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ {lk.reason || "locked"} ÃÂÃÂÃÂÃÂ· by {lk.lockedBy || "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ"} {lk.lockedAt ? `ÃÂÃÂÃÂÃÂ· ${new Date(lk.lockedAt).toLocaleString()}` : ""}
                  </div>
                )}
                {!isLocked && s.videoUrl && <div className="text-[11px] text-emerald-400/70 mt-0.5 ml-5">ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ has render ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ consider locking once approved</div>}
              </div>
      ...
      <Dialog open={!!lockDialog} onOpenChange={(o) => { if (!o) setLockDialog(null); }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Provide a reason ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ this is recorded in the audit log and shown next to the lock.</DialogDescription>
          </DialogHeader>
...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ 3-Tier Approval Chain (director ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ producer ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ exec, auto-locks on full sign-off) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
function ApprovalsTab({ projectId }: { projectId: number }) {
  ...
  const submitRejectDialog = () => {
    ...
    audit.mutate({ projectId, event: { action: "approval.rejected", targetType: "scene", targetId: String(rejectDialog.sceneId), summary: `${rejectDialog.role.toUpperCase()} rejected: ${rejectDialog.label}${note ? ` ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${note}` : ""}` } });
    setRejectDialog(null);
  };
  if (!scenes.data?.length) return <Card><CardContent className="pt-6 text-sm text-muted-foreground glass-card shadow-lg shadow-amber-500/5">No scenes yet ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ create scenes first to route them through approval.</CardContent></Card>;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <div className="text-xs text-muted-foreground">Director ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Producer ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Executive. When all three approve, scene <strong>auto-locks</strong> against further renders.</div>
      </CardHeader>
      <CardContent className="space-y-2 glass-card shadow-lg shadow-amber-500/5">
          ...
          return (
            <div key={s.id} className={`grid grid-cols-12 gap-2 min-w-[600px] items-center p-2 rounded border ${all ? "border-emerald-500/40 bg-emerald-500/5" : "border-border bg-card/50"}`}>
              <div className="col-span-4 min-w-0">
                ...
                {all && <div className="text-[10px] text-emerald-400 mt-0.5">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ fully approved ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ auto-locked</div>}
              </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="e.g. Continuity issue in shot 3, lighting needs grade passÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦"
              autoFocus
            />
          </div>
          <DialogFooter>
...
/* ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Studio Budget + Director-facing Savings Tracker ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ */
function StudioBudgetTab({ projectId }: { projectId: number }) {
  ...
  if (!live) return <div className="p-6 text-sm text-muted-foreground">Loading budgetÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦</div>;
  const update = (patch: any) => setDraft({ ...live, ...patch });
        ...
        <Card className="border-emerald-500/40 bg-gradient-to-br from-emerald-500/10 via-teal-500/5 to-transparent glass-card shadow-lg shadow-amber-500/5">
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 glass-card shadow-lg shadow-amber-500/5">
            <div>
              ...
              <div className="text-[11px] text-muted-foreground mt-1">vs ${s.tradEquivalentUsd.toLocaleString()} at industry rates ÃÂÃÂÃÂÃÂ· you spent ${live.spentUsd.toLocaleString()} ({live.spentCredits} credits)</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground flex items-center gap-1.5"><Clock className="h-3 w-3" />Time saved</div>
              <div className="text-4xl font-bold text-teal-400 gradient-text-gold">{s.timeSavedDays} <span className="text-xl">days</span></div>
              <div className="text-[11px] text-muted-foreground mt-1">{s.timeSavedHours.toLocaleString()} hours ÃÂÃÂÃÂÃÂ· {s.renderedScenes} scenes ÃÂÃÂÃÂÃÂ {live.tradHoursPerScene}h shoot day</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground flex items-center gap-1.5"><Sparkles className="h-3 w-3" />Cost multiplier</div>
              <div className="text-4xl font-bold text-violet-300 gradient-text-gold">{s.savingsMultiplier ? `${s.savingsMultiplier}ÃÂÃÂÃÂÃÂ` : "ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ"}</div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3 glass-card shadow-lg shadow-amber-500/5">
          ...
          <div className="space-y-1 pt-2">
            ...
            <div className="text-[11px] text-muted-foreground text-right">Stage sum: ${stageSum.toLocaleString()}{Math.abs(stageSum - live.totalBudget) > 1 && <span className="ml-2 text-amber-400">ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ  doesn't equal total ({live.totalBudget})</span>}</div>
          </div>
