import { useState, useMemo } from "react";
  ...
  export default function TaxIncentives() {
    ...
    <div className="max-w-5xl mx-auto space-y-6 py-6 px-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2 gradient-text-gold"><Calculator className="h-6 w-6 text-primary" />Film Tax Incentive Calculator</h1>
            ...
            <CardContent className="p-4 space-y-2">
              ...
              {top3.map((j, i) => (
                <div key={j.name} className="flex items-center gap-3 p-2 rounded-lg bg-background/60">
                  <span className="text-lg font-bold text-primary w-5">#{i + 1}</span>
              ...
              <tbody className="divide-y divide-border">
                {filtered.map(j => (
                  <tr key={j.name} className="hover:bg-muted/20 cursor-pointer transition-colors" onClick={() => setSelected(selected?.name === j.name ? null : j)}>
                    ...
                    <td className="p-3"><span className="font-bold text-primary text-base">{j.rebate}%</span></td>
