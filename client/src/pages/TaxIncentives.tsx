import { useState, useMemo } from "react";
  ...
  const JURISDICTIONS: Jurisdiction[] = [
    { name: "Georgia", country: "USA", rebate: 30, maxRebate: "Uncapped", minSpend: "$500K", type: "Tax Credit", notes: "30% transferable tax credit. Additional 10% if Georgia peach logo used. No annual cap â most popular US state for production.", url: "https://www.georgia.org/film", category: "US State" },
    { name: "New Mexico", country: "USA", rebate: 35, maxRebate: "Uncapped", minSpend: "$0", type: "Rebate", notes: "25â35% direct rebate. Higher % for qualifying spend in-state. No min spend for shorts.", url: "https://nmfilm.com", category: "US State" },
    ...
    { name: "California", country: "USA", rebate: 20, maxRebate: "$330M/yr", minSpend: "$1M", type: "Tax Credit", notes: "20â25% non-refundable. Competitive application process. Strong for TV series.", url: "https://film.ca.gov/tax-credit/", category: "US State" },
    { name: "Louisiana", country: "USA", rebate: 40, maxRebate: "$150M/yr", minSpend: "$300K", type: "Tax Credit", notes: "25â40% transferable credit. Additional 10% for Louisiana-resident payroll.", url: "https://louisianaentertainment.gov", category: "US State" },
    ...
    { name: "Texas", country: "USA", rebate: 22, maxRebate: "$95M/yr", minSpend: "$250K", type: "Grant", notes: "5â22.5% grant. Tiered by in-state spend percentage. Austin and Dallas strong production bases.", url: "https://gov.texas.gov/film", category: "US State" },
    ...
    { name: "United Kingdom", country: "UK", rebate: 34, maxRebate: "Uncapped", minSpend: "Â£1M", type: "Tax Credit", notes: "34% AVEC credit for high-end TV and film. 28% for children's TV. 40% for animation. Strong studio infrastructure.", url: "https://www.bfi.org.uk/certification-funding", category: "International" },
    ...
    { name: "Australia", country: "Australia", rebate: 20, maxRebate: "Uncapped", minSpend: "AUD$1M", type: "Rebate", notes: "16.5â20% Location Offset for qualifying foreign productions. Additional PDV offset for post-production.", url: "https://www.screenaustralia.gov.au/funding-and-support/co-productions/international-co-productions", category: "International" },
    { name: "Ireland", country: "Ireland", rebate: 32, maxRebate: "â¬70M/project", minSpend: "â¬125K", type: "Tax Credit", notes: "32% refundable Section 481 tax credit. No annual cap. Strong crew, lush locations.", url: "https://screenireland.ie/funding/the-section-481-film-relief/", category: "International" },
    { name: "France", country: "France", rebate: 30, maxRebate: "â¬30M/project", minSpend: "â¬1M", type: "Tax Credit", notes: "30% TRIP credit. Additional 20% on visual effects. Co-production treaties with 50+ countries.", url: "https://www.cnc.fr/professionnels/aides-et-financements/credit-dimpot-cinema", category: "International" },
    { name: "Germany", country: "Germany", rebate: 25, maxRebate: "â¬25M/project", minSpend: "â¬1M", type: "Tax Credit", notes: "25% DFFF incentive on German spend. Berlin, Bavaria, and Hamburg have additional state incentives.", url: "https://www.ffa.de/german-film-fund-dfff.html", category: "International" },
    ...
    { name: "Spain", country: "Spain", rebate: 30, maxRebate: "â¬10M/project", minSpend: "â¬1M", type: "Tax Credit", notes: "30% deduction on first â¬1M spend, 25% on the rest. Canary Islands offers 50%. Strong co-pro network.", url: "https://www.spainfilmcommission.org/en/production-in-spain/incentives", category: "International" },
  ...
  export default function TaxIncentives() {
    ...
    <div className="max-w-5xl mx-auto space-y-6 py-6 px-4">
              ...
              {top3.map((j, i) => (
                <div key={j.name} className="flex items-center gap-3 p-2 rounded-lg bg-background/60">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{j.type} Â· Min: {j.minSpend}</p>
                  </div>
        ...
        <div className="flex gap-3 flex-wrap">
          <Input placeholder="Search jurisdictionâ¦" value={search} onChange={e => setSearch(e.target.value)} className="max-w-xs" />
        ...
        {selected && (
          <Card className="border-primary/30 glass-card">
            <CardHeader><CardTitle className="text-base flex items-center justify-between gradient-text-gold glass-card"><span>{selected.name}, {selected.country} â {selected.rebate}% {selected.type}</span><a href={selected.url} target="_blank" rel="noopener noreferrer"><Button variant="outline" size="sm" className="gap-1 text-xs"><ExternalLink className="h-3 w-3" />Official Site</Button></a></CardTitle></CardHeader>
