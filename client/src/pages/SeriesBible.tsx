      ...
      return (
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2 space-y-1.5"><Label>Logline</Label><Textarea className="h-16 focus:ring-amber-500/30 focus:border-amber-500/50" placeholder="One-sentence description of the series…" value={form.logline} onChange={e => setEditingSeries(p => ({ ...(p ?? form), logline: e.target.value }))} /></div>
            <div className="sm:col-span-2 space-y-1.5"><Label>Premise</Label><Textarea className="h-24 focus:ring-amber-500/30 focus:border-amber-500/50" placeholder="Expand on the central concept, conflict, and world of the series…" value={form.premise} onChange={e => setEditingSeries(p => ({ ...(p ?? form), premise: e.target.value }))} /></div>
            <div className="sm:col-span-2 space-y-1.5"><Label>World Building</Label><Textarea className="h-24 focus:ring-amber-500/30 focus:border-amber-500/50" placeholder="Setting, rules, history, and atmosphere of the show's world…" value={form.worldBuilding} onChange={e => setEditingSeries(p => ({ ...(p ?? form), worldBuilding: e.target.value }))} /></div>
            <div className="sm:col-span-2 space-y-1.5"><Label>Main Characters</Label><Textarea className="h-24 focus:ring-amber-500/30 focus:border-amber-500/50" placeholder="List and describe your central characters and their arcs across the series…" value={form.mainCharacters} onChange={e => setEditingSeries(p => ({ ...(p ?? form), mainCharacters: e.target.value }))} /></div>
            <div className="sm:col-span-2 space-y-1.5"><Label>Tone & Style</Label><Textarea className="h-20 focus:ring-amber-500/30 focus:border-amber-500/50" placeholder="Describe the visual language, pacing, tone, and comparable shows…" value={form.toneAndStyle} onChange={e => setEditingSeries(p => ({ ...(p ?? form), toneAndStyle: e.target.value }))} /></div>
          </div>
