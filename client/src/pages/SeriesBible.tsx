              ...
              {activeSeries.seasons.map(season => (
                <div key={season.id}>
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-semibold flex items-center gap-2"><Layers className="h-4 w-4 text-amber-400" />Season {season.number}</h3>
                          ...
                          <div className="flex-1 min-w-0">
                            {editingEpisode?.ep.id === ep.id ? (
                              <div className="space-y-2">
                                ...
                                <Textarea placeholder="Episode loglineÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦" value={editingEpisode.ep.logline} onChange={e => setEditingEpisode(prev => prev ? { ...prev, ep: { ...prev.ep, logline: e.target.value } } : null)} className="h-16 text-xs" />
      ...
      return (
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2 space-y-1.5"><Label>Logline</Label><Textarea className="h-16" placeholder="One-sentence description of the seriesÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦" value={form.logline} onChange={e => setEditingSeries(p => ({ ...(p ?? form), logline: e.target.value }))} /></div>
            <div className="sm:col-span-2 space-y-1.5"><Label>Premise</Label><Textarea className="h-24" placeholder="Expand on the central concept, conflict, and world of the seriesÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦" value={form.premise} onChange={e => setEditingSeries(p => ({ ...(p ?? form), premise: e.target.value }))} /></div>
            <div className="sm:col-span-2 space-y-1.5"><Label>World Building</Label><Textarea className="h-24" placeholder="Setting, rules, history, and atmosphere of the show's worldÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦" value={form.worldBuilding} onChange={e => setEditingSeries(p => ({ ...(p ?? form), worldBuilding: e.target.value }))} /></div>
            <div className="sm:col-span-2 space-y-1.5"><Label>Main Characters</Label><Textarea className="h-24" placeholder="List and describe your central characters and their arcs across the seriesÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦" value={form.mainCharacters} onChange={e => setEditingSeries(p => ({ ...(p ?? form), mainCharacters: e.target.value }))} /></div>
            <div className="sm:col-span-2 space-y-1.5"><Label>Tone & Style</Label><Textarea className="h-20" placeholder="Describe the visual language, pacing, tone, and comparable showsÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦" value={form.toneAndStyle} onChange={e => setEditingSeries(p => ({ ...(p ?? form), toneAndStyle: e.target.value }))} /></div>
          </div>
            ...
            return (
              <Card key={s.id} className="cursor-pointer hover:border-primary/50 transition-colors group glass-card shadow-lg shadow-amber-500/5" onClick={() => setActiveSeries(s)}>
                <CardContent className="p-4 space-y-2 glass-card shadow-lg shadow-amber-500/5">
                  ...
                  <p className="text-xs text-muted-foreground">{s.seasons.length} season{s.seasons.length !== 1 ? "s" : ""} ÃÂÃÂÃÂÃÂ· {totalEps} episode{totalEps !== 1 ? "s" : ""}</p>
                </CardContent>
              </Card>
            );
          })}
