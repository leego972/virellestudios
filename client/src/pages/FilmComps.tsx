  ...
  export default function FilmComps() {
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><BarChart3 className="h-6 w-6 text-primary" />Film Comps & Market Analytics</h1>
              ...
              <tbody className="divide-y divide-border">
                {filtered.map(film => (
                  <tr key={film.title} className={`hover:bg-muted/20 cursor-pointer transition-colors ${selected?.title === film.title ? "bg-primary/5" : ""}`} onClick={() => setSelected(selected?.title === film.title ? null : film)}>
                    ...
                    <td className="p-3"><span className={`font-bold ${film.roi >= 20 ? "text-green-500" : film.roi >= 5 ? "text-primary" : "text-muted-foreground"}`}>{film.roi}x</span></td>
