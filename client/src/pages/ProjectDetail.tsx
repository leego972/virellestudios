...
export default function ProjectDetail() {
  ...
  const quickGenMutation = trpc.generation.quickGenerate.useMutation({
    onSuccess: (result) => {
      ...
      toast.success("Film generation started! This runs in the background ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ check back in a few minutes.");
    },
    onError: (err) => toast.error(err.message),
  });


  // ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Generate Full Feature Film (60-90 min) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
  ...
  const generateFullFilmMutation = trpc.generation.generateFullFilm.useMutation({
    onSuccess: () => {
      toast.success("ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¬ Full film generation started! This will take some time ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ you'll be notified when complete.");
      utils.project.get.invalidate({ id: projectId });
  ...
  const generateSceneMutation = trpc.scene.generateVideo.useMutation({
    onSuccess: (result: any) => {
      ...
      if (result?.status === "generating") {
        toast.success("Scene regeneration started ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ check back in 2ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ5 minutes.");
      } else {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
              ...
              {project.rating && <><span>ÃÂÃÂÃÂÃÂ·</span><span>{project.rating}</span></>}
              {project.genre && <><span>ÃÂÃÂÃÂÃÂ·</span><span>{project.genre}</span></>}
              {editingDuration ? (
                ...
                <>
                  {project.duration && (
                    <span className="cursor-pointer hover:text-amber-400 transition-colors" title="Click to edit duration" onClick={() => { setDurationInput(String(project.duration)); setEditingDuration(true); }}>
                      <span>ÃÂÃÂÃÂÃÂ·</span> {project.duration < 2 ? `${Math.round(project.duration * 60)}s` : `${project.duration} min`} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
                    </span>
                  )}
                  {!project.duration && scenes?.length ? (
                    <span className="cursor-pointer hover:text-amber-400 transition-colors" title="Click to set duration" onClick={() => { setDurationInput("5"); setEditingDuration(true); }}>
                      <span>ÃÂÃÂÃÂÃÂ·</span> {(() => { const totalSec = (scenes || []).reduce((sum: number, s: any) => sum + (s.duration || 30), 0); return totalSec < 120 ? `${totalSec}s` : `${Math.round(totalSec / 60)} min`; })()} ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
                    </span>
                  ) : !project.duration ? (
                    <span className="cursor-pointer hover:text-amber-400 transition-colors text-muted-foreground" title="Click to set duration" onClick={() => { setDurationInput("5"); setEditingDuration(true); }}>
                      <span>ÃÂÃÂÃÂÃÂ·</span> Set duration ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
                    </span>
                  ) : null}
                </>
              )}
            ...
            onClick={async () => {
              try {
                const reviewerName = window.prompt(
                  "Reviewer name (optional)\n\nFor watermarked screeners ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ leave blank for an unwatermarked link, or enter a name (e.g. \"Studio A24\", \"Jane Producer\") to brand the screener with their name + timestamp on every frame. Pro tip: use a different name per recipient so leaks can be traced.",
                  "",
                ) || "";
                ...
                toast.success(
                  reviewerName.trim()
                    ...
                    : "Review link copied ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ paste it to share with producers, friends or collaborators.",
                );
              } catch (e: any) {
            ...
            <Button
              ...
              onClick={() => {
                if (project.status === "completed") {
                  ...
                  // draft OR failed ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ kick off generation immediately (no confirm needed ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
      ...
      {/* Generation Progress */}
      {/* Generation Error Banner ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ surfaces the ACTUAL backend error message instead of a
      ...
      {jobs && jobs.some((j: any) => j.errorMessage) && project.status !== "generating" && (() => {
        ...
        // video key (e.g. fal.ai) is fine ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ it's the script-generation step that died.
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-red-400 break-words">
                    {isNoVideoKey ? "Add a video-generation API key to continue"
                      : isLLMExhausted ? "Script generation paused ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ AI text quota reached"
                  ...
                  {isNoVideoKey ? (
                    <>
                      <p className="text-xs text-red-300/80 mt-1 break-words">
                        You haven't connected a video provider yet. Virelle Studios is a Bring-Your-Own-Key platform for video ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ add a key once and you can generate as many films as your provider's credits allow.
                      </p>
                      <p className="text-[11px] text-muted-foreground mt-2">
                        Open <a href="/settings" className="font-medium text-foreground underline">Settings ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ API Keys</a> and connect at least one of:
                        <span className="block mt-1.5 ml-2 leading-relaxed">
                          ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ <span className="text-foreground font-medium">fal.ai</span> ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ cheapest, ~$0.40/clip (recommended)<br/>
                          ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ <span className="text-foreground font-medium">Runway</span> ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ best quality, ~$0.05ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ0.10/sec<br/>
                          ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ <span className="text-foreground font-medium">Hugging Face</span> ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ free tier (300 req/hr)<br/>
                          ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ <span className="text-foreground font-medium">Luma ÃÂÃÂÃÂÃÂ· Replicate ÃÂÃÂÃÂÃÂ· Google Veo 3</span> ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ alternatives
                        </span>
                    ...
                    <>
                      <p className="text-xs text-red-300/80 mt-1 break-words">
                        The shared text-AI used to write your script and scene prompts has hit its temporary usage cap. This is <span className="font-medium">not</span> your fal.ai / video key ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ that's still fine.
                      </p>
                      <p className="text-[11px] text-muted-foreground mt-2">
                        Quickest fix: open <a href="/settings" className="font-medium text-foreground underline">Settings ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ API Keys</a> and add your own <span className="font-medium text-foreground">Venice AI</span> (cheapest, ~$5 = millions of tokens), <span className="font-medium text-foreground">OpenAI</span>, or <span className="font-medium text-foreground">Anthropic</span> key ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ script generation will use it automatically. Otherwise wait a few minutes and tap <span className="font-medium text-foreground">Retry Generation</span> below ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ the shared cap usually clears within the hour.
                      </p>
                    </>
                  ) : isVideoQuota ? (
                    <>
                    ...
                    <>
                      <p className="text-xs text-red-300/80 mt-1 break-words whitespace-pre-wrap">{rawMsg}</p>
                      <p className="text-[11px] text-muted-foreground mt-2">
                        If your API keys are funded and connected (<a href="/settings" className="font-medium text-foreground underline">Settings ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ API Keys</a>), this is usually a temporary provider issue ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ tap <span className="font-medium text-foreground">Retry Generation</span> below to retry. Make sure your preferred provider is selected.
                      </p>
                    </>
                  )}

                  {/* Inline action buttons ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ always present so the user is never stranded */}
                  <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-red-500/20">
                    ...
                    >
                      {quickGenMutation.isPending ? (
                        <><Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin text-amber-400" />RetryingÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦</>
                      ) : (
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        <TabsList className="bg-card/50 grid grid-cols-3 sm:flex sm:flex-wrap h-auto gap-1.5 p-1.5 w-full sm:justify-start [&>*]:bg-background/40 [&>*]:border [&>*]:border-border/40 [&>*]:data-[state=active]:bg-amber-500/15 [&>*]:data-[state=active]:border-amber-400/40 [&>*]:data-[state=active]:text-amber-400">
          {/* Pipeline order: Overview ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Story ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Characters ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Scenes ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Soundtrack ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Trailer ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Export ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Tools */}
        ...
        {/* Journey Tab ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ the unified filmmaker pipeline */}
            ...
            <Card className="bg-card/50 glass-card shadow-lg shadow-amber-500/5">
              <CardContent className="p-4">
                ...
                >
                  ...
                  {/* Play button overlay ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ show when full film or scene videos exist */}
                    ...
                    <CardContent className="p-3">
                      ...
                      <div className="flex items-center gap-1 mt-0.5">
                        ...
                        <p className="text-xs text-muted-foreground truncate">
                          {[attrs.role, attrs.age || attrs.ageRange, attrs.gender].filter(Boolean).join(" ÃÂÃÂÃÂÃÂ· ")}
                        </p>
                      </div>
                      <button
                  ...
                  <CardContent className="p-3 flex items-center gap-3">
                    ...
                    {scenes && scenes.length > 0 && (
                      <span className="absolute top-2 left-2 text-[10px] text-white/80 bg-black/50 rounded px-1.5 py-0.5">
                        {scenes.length} scenes ÃÂÃÂÃÂÃÂ· ~{Math.round(scenes.length * 1.5)} min est.
                      </span>
                    )}                        <p className="text-sm font-medium truncate">{scene.title || "Untitled Scene"}</p>
                      </div>
                      <p className="text-xs text-muted-foreground mt-0.5 truncate">
                        {[scene.timeOfDay, scene.locationType, scene.mood].filter(Boolean).join(" ÃÂÃÂÃÂÃÂ· ")}
                      </p>
                      {isFailed && (
                        <p className="text-[11px] text-red-400 mt-0.5">Generation failed ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ click Retry to regenerate this scene</p>
                      )}
                    </div>
                ...
                <Card key={track.id} className="bg-card/50 group glass-card shadow-lg shadow-amber-500/5">
                  <CardContent className="p-4 flex items-center gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5 truncate">
                        {[track.artist, track.genre, track.mood].filter(Boolean).join(" ÃÂÃÂÃÂÃÂ· ")}
                      </p>
                    </div>
          ...
          <Card className="bg-card/50 glass-card shadow-lg shadow-amber-500/5">
            ...
            <CardContent className="space-y-5">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      AI-generated signing interpreter in a circle overlay ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ optional, requires a D-ID API key in your BYOK settings
                    </p>
                  </div>
                  <Switch
                ...
                {/* Auslan position ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ only shown when Auslan is enabled */}
              ...
              <Card className="cursor-pointer hover:ring-1 hover:ring-amber-500/40 transition-all border-amber-500/20 glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/schedule`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Shoot days ÃÂÃÂÃÂÃÂ· scene assignment ÃÂÃÂÃÂÃÂ· ordering</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/contacts`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Department directory ÃÂÃÂÃÂÃÂ· call overrides</p>
                  </div>
                </CardContent>
              </Card>
              <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/budget-tracker`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Estimate vs actual ÃÂÃÂÃÂÃÂ· category tracking</p>
                  </div>
                </CardContent>
              </Card>
              <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/activity`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Approvals ÃÂÃÂÃÂÃÂ· changes ÃÂÃÂÃÂÃÂ· audit trail</p>
                  </div>
                </CardContent>
              </Card>
              <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/collaborators`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Invite ÃÂÃÂÃÂÃÂ· roles ÃÂÃÂÃÂÃÂ· permissions</p>
                  </div>
                </CardContent>
              </Card>
              <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/approvals`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Cryptographic audit ÃÂÃÂÃÂÃÂ· sha256</p>
                  </div>
                </CardContent>
              </Card>
              <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/script-import`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Fountain ÃÂÃÂÃÂÃÂ· Final Draft FDX</p>
                  </div>
                </CardContent>
              </Card>
              <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/script-export`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Fountain ÃÂÃÂÃÂÃÂ· FDX downloads</p>
                  </div>
                </CardContent>
              </Card>
              <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/calendar-feed`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">iCal ÃÂÃÂÃÂÃÂ· Google ÃÂÃÂÃÂÃÂ· Apple subscribe</p>
                  </div>
                </CardContent>
              </Card>
              <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/budget-fringes`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Union ÃÂÃÂÃÂÃÂ· payroll ÃÂÃÂÃÂÃÂ· taxes</p>
                  </div>
                </CardContent>
              </Card>
              <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/asset-versions`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Stack ÃÂÃÂÃÂÃÂ· diff ÃÂÃÂÃÂÃÂ· revert</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/daily-report`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Wrap-of-day DPR ÃÂÃÂÃÂÃÂ· signatures</p>
                  </div>
                </CardContent>
              </Card>
            ...
            <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all border-amber-500/30 bg-amber-400/5 glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/pre-production`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Vision ÃÂÃÂÃÂÃÂ· Locations ÃÂÃÂÃÂÃÂ· Vehicles ÃÂÃÂÃÂÃÂ· Atmosphere</p>
                  </div>
                </CardContent>
              </Card>
            ...
            >
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Generate all {scenes?.length || 0} scenes automatically ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ dialogue, cinematics, soundtrack, full assembly
                  </p>
                </div>
            ...
            <Card className="cursor-pointer hover:ring-2 hover:ring-primary/60 transition-all border-primary/40 bg-amber-400/5 glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/director-cut`)}>
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Timeline editor ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ trim, reorder &amp; retake scenes</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/subtitles`)}>
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">130+ languages ÃÂÃÂÃÂÃÂ· AI generation & translation</p>
                </div>
              </CardContent>
            </Card>
            <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/sound-effects`)}>
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">ADR ÃÂÃÂÃÂÃÂ· Foley ÃÂÃÂÃÂÃÂ· Score ÃÂÃÂÃÂÃÂ· Mix ÃÂÃÂÃÂÃÂ· SFX library</p>
                </div>
              </CardContent>
            </Card>
            <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/visual-effects`)}>
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">VFX library ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ explosions, weather, magic & more</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="cursor-pointer hover:ring-2 hover:ring-amber-500/60 transition-all border-amber-500/40 bg-gradient-to-br from-amber-500/10 to-rose-500/5 glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/pro-studio`)}>
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">Style bible ÃÂÃÂÃÂÃÂ· Shot versions ÃÂÃÂÃÂÃÂ· Voice/likeness rights ÃÂÃÂÃÂÃÂ· C2PA provenance ÃÂÃÂÃÂÃÂ· Render economics</p>
                </div>
              ...
              <Card className="cursor-pointer hover:ring-2 hover:ring-amber-500/60 transition-all border-amber-500/40 bg-gradient-to-br from-amber-500/10 to-rose-500/5 glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/pro-studio`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">Style bible ÃÂÃÂÃÂÃÂ· Shot versions ÃÂÃÂÃÂÃÂ· Voice rights ÃÂÃÂÃÂÃÂ· C2PA ÃÂÃÂÃÂÃÂ· Render economics</p>
                  </div>
                  <Badge className="bg-amber-400/20 text-amber-400 border-amber-500/30 text-[10px]">PRO</Badge>
                </CardContent>
              </Card>
              <Card className="cursor-pointer hover:ring-2 hover:ring-violet-500/60 transition-all border-violet-500/40 bg-gradient-to-br from-violet-500/10 to-fuchsia-500/5 glass-card shadow-lg shadow-amber-500/5" onClick={() => setLocation(`/projects/${project.id}/studio-ops`)}>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">Frame reviews ÃÂÃÂÃÂÃÂ· Color ÃÂÃÂÃÂÃÂ· Versions ÃÂÃÂÃÂÃÂ· Render queue ÃÂÃÂÃÂÃÂ· Deliverables ÃÂÃÂÃÂÃÂ· Clearances ÃÂÃÂÃÂÃÂ· Distribution ÃÂÃÂÃÂÃÂ· Audit ÃÂÃÂÃÂÃÂ· Proxies ÃÂÃÂÃÂÃÂ· Cuts</p>
                  </div>
      ...
      <Dialog open={fullFilmDialogOpen} onOpenChange={setFullFilmDialogOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="space-y-2">
              {[
                ['generateDialogue', 'Generate Voice Acting & Dialogue', 'ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ'],
                ['generateSoundtrack', 'Generate AI Film Score', 'ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂµ'],
                ['useCharacterConsistency', 'Character Consistency (LoRA)', 'ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¤'],
                ['useSceneContinuity', 'Scene-to-Scene Continuity Chain', 'ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ'],
              ].map(([key, label, emoji]) => (
      ...
      {/* Professional Media Player ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ scene preview */}
      {/* Professional Media Player ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ scene preview */}
      ...
      {/* Professional Media Player ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ full stitched film */}
...
// ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Story Editor Component ÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
      ...
      <Card className="bg-card/50 glass-card shadow-lg shadow-amber-500/5">
        ...
        <CardContent className="space-y-3">
          <div className="space-y-1">
            ...
            <Textarea
              placeholder="Describe the main storyline in detail ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ the central conflict, protagonist's journey, and how events unfold..."
              value={mainPlot}
          ...
          <div className="space-y-1">
            ...
            <Textarea
              placeholder="Secondary storylines ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ romance, rivalry, mystery, parallel journeys..."
              value={sidePlots}
      ...
      <Card className="bg-card/50 glass-card shadow-lg shadow-amber-500/5">
        ...
        <CardContent className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            <Textarea
              placeholder="How does the film begin? The first thing the audience sees ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ set the tone, introduce the world..."
              value={openingScene}
          ...
          <div className="space-y-1">
            ...
            <Textarea
              placeholder="The peak of tension ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ the central conflict comes to a head. What happens? Who is involved?"
              value={climax}
...
/**
 * Reviews inbox ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ surfaces public reviewer comments left on shared
