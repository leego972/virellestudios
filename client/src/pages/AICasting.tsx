...
// âââ VIRELLE SIGNATURE CAST ââââââââââââââââââââââââââââââââââââââââââââââââ
const SIGNATURE_CAST = [
  ...
  },
  {
    ...
    badges: ["Family Unit â Vance"],
    unlocked: true,
  },
];

// âââ GENERIC ARCHETYPE LIBRARY (secondary) ââââââââââââââââââââââââââââââââ
const AI_ACTOR_LIBRARY = [
  ...
  { id: "actor-006", name: "Yuki Tanaka", archetype: "Quiet Intensity", age: "20-30", ethnicity: "Japanese", specialty: "Drama / Horror", rating: 4.6, uses: 5430, description: "Says little but observes everything. Still waters run deep â when she acts, it is decisive and irreversible." },
...
function AICastingInner() {
  ...
  const enrichedSignatureCast = useMemo(() => {
    return SIGNATURE_CAST.map((actor) => {
      // Map sig- prefixed IDs to server IDs (sig-julian-vance â julian-vance)
      ...
      const results = await Promise.allSettled(
        selectedActors.map((id) => {
          ...
          const sigNote = actor.isSignature
            ? `[Virelle Signature Cast â ${actor.tier === "flagship" ? "Flagship Star" : actor.tier === "premium" ? "Premium Cast" : "Standard"} â Archetype: ${actor.archetype}]`
            : `[Cast from AI Actor Library â Archetype: ${actor.archetype}, Specialty: ${actor.specialty}]`;
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {selectedActors.length > 0
                  ? `${selectedActors.length} actor${selectedActors.length > 1 ? "s" : ""} selected â set performance direction, then cast`
...
// âââ SIGNATURE ACTOR CARD ââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââ UNLOCK MODAL âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
      ...
      <div className="bg-zinc-900 border border-zinc-700/60 rounded-xl max-w-md w-full p-6 space-y-5">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-xs text-amber-400/80 mt-0.5">{actor.archetype} â <TierBadge tier={actor.tier} /></p>
          </div>
        ...
        <p className="text-sm text-zinc-400">
          ...
          Virelle Stars are licensed for professional cinematic use â not pornography.
        </p>
        <div className="space-y-2">
          <button
