...
import { Eye, EyeOff, Loader2, ArrowRight, UserPlus, Store } from "lucide-react";
...
export default function Login() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      if (designerMode) {
        const result = await designerLogin.mutateAsync({ email: email.trim(), password });
        toast.success("Designer portal ready.");
        window.location.assign(result.redirect || "/designer/studio");
        return;
      }
        ...
        <div className="w-full max-w-sm space-y-6">
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {designerMode ? "Designer marketplace portal" : "Create Hollywood grade productions with AI"}
              </p>
            </div>
          </div>
          ...
          <Card className="border-border/50 bg-card/80 backdrop-blur-sm shadow-xl glass-card shadow-lg shadow-amber-500/5 hover:shadow-amber-500/20 transition-shadow gold-glow">
            <CardHeader className="space-y-1 pb-4">
              <CardTitle className="text-xl text-center gradient-text-gold flex items-center justify-center gap-2">
                {designerMode && <Store className="w-5 h-5 text-amber-400" />}
                {designerMode ? "Designer Sign In" : "Sign In"}
              </CardTitle>
              <CardDescription className="text-center">
                {designerMode
                  ? "Designer-only access to listings, Stripe payouts and fulfilment"
                ...
                <div className="w-full pt-2">
                  <div className="rounded-lg border border-amber-500/20 bg-amber-600/5 p-3.5">
                    <p className="text-sm text-muted-foreground text-center mb-2.5">
                      {designerMode ? "Need a designer account?" : "Don't have an account?"}
                    </p>
                    <Link href={designerMode ? "/designer-register" : "/register"} className="block">
                      <Button
                        type="button"
                        variant="outline"
