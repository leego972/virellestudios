import { useState, useEffect, useCallback } from "react";
  ...
  const ELEMENT_TYPES = [
    ...
    { id: "chapter_marker", label: "Chapter Marker", icon: LayoutTemplate, description: 'PART I ÃÂ· CHAPTER 3 ÃÂ· ACT TWO', color: "text-blue-400", bg: "bg-blue-500/10", border: "border-blue-500/20", defaultDuration: 4 },
    ...
    { id: "time_location", label: "Time & Location", icon: MapPin, description: "Paris, France Ã¢ÂÂ 1943", color: "text-cyan-400", bg: "bg-cyan-500/10", border: "border-cyan-500/20", defaultDuration: 5 },
    ...
    return (
      <div className="flex flex-col h-full gap-3">
        ...
        <div className="flex items-center justify-between gap-2">
          <Button variant="outline" size="sm" onClick={() => setCurrent(c => Math.max(0, c - 1))} disabled={current === 0}>Ã¢ÂÂ¹</Button>
          <div className="text-xs text-muted-foreground flex-1 text-center">
            {current + 1} / {elements.length} Ã¢ÂÂ <span className={typeInfo?.color}>{typeInfo?.label}</span>
          </div>
          <Button variant="outline" size="sm" onClick={() => setCurrent(c => Math.min(elements.length - 1, c + 1))} disabled={current === elements.length - 1}>Ã¢ÂÂº</Button>
        </div>
      </div>
    );
  }
  ...
  export default function OpeningSequence() {
    ...
    const saveToProject = () => {
      const sequenceData = JSON.stringify({
        ...
        generationNotes: elements.map(e =>
          `[${ELEMENT_TYPES.find(t=>t.id===e.type)?.label}] ${e.duration}s: ${e.text || e.type} Ã¢ÂÂ ${e.generationPrompt || e.notes || ""}`
        ).join("\n"),
      });
    ...
    const exportScript = () => {
      const lines: string[] = [
        "OPENING SEQUENCE Ã¢ÂÂ PRODUCTION NOTES",
        "=" .repeat(50),
      ...
      elements.forEach((el, i) => {
        const typeInfo = ELEMENT_TYPES.find(t => t.id === el.type)!;
        lines.push(`\n${String(i+1).padStart(2,"0")}. [${typeInfo.label}] Ã¢ÂÂ ${el.duration}s Ã¢ÂÂ ${el.animation}`);
      ...
      lines.push("\n" + "=".repeat(50));
      lines.push("Generated by Virelle Studios ÃÂ· virelle.life");
    ...
    return (
      <div className="min-h-screen" style={{ background: "linear-gradient(135deg, #0a0a0f 0%, #0f0f1a 50%, #0a0a0f 100%)" }}>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-muted-foreground tracking-widest uppercase">
                    {(project.data as any)?.title || "Loading..."} ÃÂ· {elements.length} elements ÃÂ· {totalDuration}s
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          {/* Info banner Ã¢ÂÂ pipeline connection */}
                        ...
                        {selected.type === "narrator_vo" || selected.type === "star_wars_crawl" ? (
                          ...
                          <Input
                            ...
                            placeholder={
                              ...
                              selected.type === "content_warning" ? "RATED R Ã¢ÂÂ STRONG LANGUAGE" :
