...
function describe(e: any): string {
  const p = e.payload || {};
  switch (e.eventType) {
    case "scene.approval.set": return `Marked scene ${p.sceneId ?? ""} as ${p.status}${p.note ? ` â "${p.note}"` : ""}`;
    case "movie.approval.set": return `Marked cut ${p.movieId ?? ""} as ${p.status}${p.note ? ` â "${p.note}"` : ""}`;
    ...
    case "budget.update": return `Updated budget â total ${p.currency} ${p.totalEstimate}`;
    default: return e.eventType;
  }
}

/** v6.63 â Project activity timeline. Append-only audit feed. */
    ...
    <div className="min-h-screen text-zinc-100 p-4 md:p-6" style={{ background:"linear-gradient(135deg,#07070e 0%,#0c0b18 60%,#07070a 100%)" }}>
      <div className="max-w-3xl mx-auto space-y-4">
        <div>
          ...
          <p className="text-sm text-zinc-400">{project?.title || "â"} Â· last 200 events.</p>
        </div>

        {isLoading ? (
          <div className="text-center py-12 text-zinc-500"><Loader2 className="w-5 h-5 animate-spin inline mr-2 text-amber-400" />Loadingâ¦</div>
        ) : (events as any[]).length === 0 ? (
          <Card className="bg-zinc-950 border-zinc-800 glass-card shadow-lg shadow-amber-500/5"><CardContent className="p-8 text-center text-sm text-zinc-500 glass-card shadow-lg shadow-amber-500/5">No activity yet. Approvals, schedule changes, and crew updates will show up here.</CardContent></Card>
        ) : (
          <div className="space-y-2">
            {(events as any[]).map((e) => {
              ...
              return (
                <Card key={e.id} className="bg-zinc-950 border-zinc-800 glass-card shadow-lg shadow-amber-500/5">
                  <CardContent className="p-3 flex items-start gap-3 glass-card shadow-lg shadow-amber-500/5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-zinc-500 mt-0.5">
                        {e.actor || "User"} Â· {new Date(e.createdAt).toLocaleString()}
                      </div>
                    </div>
                  </CardContent>
                </Card>
