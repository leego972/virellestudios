...
function describe(e: any): string {
  const p = e.payload || {};
  switch (e.eventType) {
    case "scene.approval.set": return `Marked scene ${p.sceneId ?? ""} as ${p.status}${p.note ? ` Ã¢ÂÂ "${p.note}"` : ""}`;
    case "movie.approval.set": return `Marked cut ${p.movieId ?? ""} as ${p.status}${p.note ? ` Ã¢ÂÂ "${p.note}"` : ""}`;
    ...
    case "budget.update": return `Updated budget Ã¢ÂÂ total ${p.currency} ${p.totalEstimate}`;
    default: return e.eventType;
  }
}

/** v6.63 Ã¢ÂÂ Project activity timeline. Append-only audit feed. */
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {(events as any[]).map((e) => {
              ...
              return (
                <Card key={e.id} className="bg-zinc-950 border-zinc-800 glass-card shadow-lg shadow-amber-500/5 hover:shadow-amber-500/20 transition-shadow">
                  <CardContent className="p-3 flex items-start gap-3 glass-card shadow-lg shadow-amber-500/5 hover:shadow-amber-500/20 transition-shadow">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-zinc-500 mt-0.5">
                        {e.actor || "User"} ÃÂ· {new Date(e.createdAt).toLocaleString()}
                      </div>
                    </div>
                  </CardContent>
                </Card>
