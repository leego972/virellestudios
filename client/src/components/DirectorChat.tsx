...
// âââ Preset edit commands âââ
const EDIT_PRESETS = [
...
// âââ Animated voice waveform âââ
...
export default function DirectorChat({ projectId, defaultOpen = false, hideVoiceOverlay = false }: DirectorChatProps) {
  ...
  // VAD (Voice Activity Detection) refs â Titan-style auto-stop on silence
  ...
  // âââ Web Speech API (free, on-device transcription) âââ
  ...
  // âââ Floating window state âââ
  ...
  // âââ SSE-based send with tool-calling âââ
  const sendViaSSE = useCallback(async (
    ...
    es.onopen = async () => {
      ...
      try {
        ...
        toast.error("Your message didn't reach the Director â please try again.");
      }
    };
    ...
    es.addEventListener("done", (e: MessageEvent) => {
      try {
        ...
        es.close();
        sseRef.current = null;
        // In voice mode â auto-speak the response
  ...
  // âââ Scroll to bottom with ResizeObserver âââ
  ...
  // âââ iOS Safari: lock panel to visual viewport so keyboard doesn't jump the UI âââ
  ...
  // âââ Draggable window: mouse + touch drag on the header âââ
  ...
  // âââ Sync history to local messages âââ
  useEffect(() => {
  ...
  // âââ Strip markdown for TTS (Titan-style) âââ
  ...
  // âââ AudioContext TTS (Safari iOS safe â Titan pattern) âââ
  ...
  // âââ Full-screen Voice Mode â Titan-style with VAD âââ
    ...
    startVoiceModeRecordingRef.current = async () => {
      ...
      try {
        ...
        recorder.onstop = async () => {
          ...
          // âââ Prefer Web Speech API result if we got one âââ
              ...
              { projectId: projectId ?? 0, audioData: base64, mimeType: baseMime as "audio/mpeg" | "audio/wav" | "audio/mp4" | "audio/webm" | "audio/ogg" | "audio/aac" },
              {
                onSuccess: (data) => {
                  ...
                  if (data.text?.trim()) {
                    ...
                    setVoiceModeTranscript("Didn't catch that â try again");
                    setVoiceModeState("listening");
                    voiceModeStateRef.current = "listening";
                    startVoiceModeRecordingRef.current();
                  }
                ...
                onError: () => {
                  if (!voiceModeRef.current) return;
                  setVoiceModeTranscript("Transcription failed â try again");
                  setVoiceModeState("listening");
                  voiceModeStateRef.current = "listening";
                  startVoiceModeRecordingRef.current();
                },
        ...
        // âââ Web Speech API: start in PARALLEL with MediaRecorder âââ
        // Free, on-device, no quota. If it returns a transcript first, we use it
        // and skip the uploadâWhisper roundtrip. Failures are silent (we still have MediaRecorder).
        ...
        if (webSpeechSupported) {
          try {
            ...
            recog.onerror = (e: any) => {
              ...
              // Don't clear webSpeechTextRef â partial text may still be usable
            };
        ...
        // âââ VAD: Titan-style auto-stop on silence âââ
  ...
  const stopVoiceModeRecording = useCallback(() => {
    ...
    // Note: webSpeechRef is intentionally NOT stopped here â recorder.onstop will read
  ...
  // âââ Standard voice recording (inline, for dictation/edit) âââ
  ...
  // âââ Copy message âââ
  const copyMessage = useCallback((text: string, idx: number) => {
    navigator.clipboard.writeText(text).then(() => {
      setCopiedIdx(idx);
      setTimeout(() => setCopiedIdx(null), 1500);
    }).catch(() => toast.error("Couldn't copy to clipboard â try selecting the text manually."));
  }, []);

  // âââ Regenerate last response âââ
  ...
  return (
    <>
      {/* âââ Floating trigger pill â only visible when chat is fully closed âââ */}
      {!isOpen && (
        <button
      ...
      {/* âââ Minimized pill â visible when open but minimized âââ */}
      {isOpen && isMinimized && (
        <button
          onClick={() => setIsMinimized(false)}
          className="fixed bottom-6 right-6 z-50 flex items-center gap-2.5 rounded-full pl-3 pr-4 py-2.5 shadow-2xl transition-all duration-200 hover:scale-105 active:scale-95 bg-[#07070e] border border-amber-500/40"
          style={{ touchAction: "manipulation" }}
        >
      ...
      {/* âââ Full-screen Voice Mode Overlay âââ */}
          ...
          <div className="mt-8 flex flex-col items-center gap-3 px-6 max-w-[320px] w-full">
            <div className="flex items-center gap-2">
              <div className={cn(
                ...
                voiceModeState === 'speaking'  ? 'bg-emerald-400 animate-pulse' : 'bg-[rgba(255,255,255,0.06)]/20'
              )} />
              <span className={cn(
      ...
      >
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {voiceModeActive && voiceModeState === "listening" && <span className="text-amber-400">Listeningâ¦</span>}
                {voiceModeActive && voiceModeState === "thinking" && <span className="text-blue-400">Thinkingâ¦</span>}
                {voiceModeActive && voiceModeState === "speaking" && <span className="text-emerald-400">Speakingâ¦</span>}
          ...
          <div className="flex items-center gap-1" onMouseDown={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()}>
            ...
            {/* Minimize â collapses to pill, stays active */}
            <Button
              variant="ghost"
              size="icon"
              className="size-8"
        ...
        {showInstructions && (
          <div className="px-4 py-3 border-b bg-[rgba(255,255,255,0.04)]/20 shrink-0">
        ...
        {showShortcutHint && (
          <div className="px-4 py-2.5 border-b bg-[rgba(255,255,255,0.04)]/30 text-xs space-y-1 shrink-0">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-muted-foreground">
              <span><kbd className="px-1 py-0.5 rounded bg-[#07070e] border border-border text-[10px] font-mono">V</kbd> Start recording</span>
              <span><kbd className="px-1 py-0.5 rounded bg-[#07070e] border border-border text-[10px] font-mono">S</kbd> Stop recording</span>
              <span><kbd className="px-1 py-0.5 rounded bg-[#07070e] border border-border text-[10px] font-mono">Esc</kbd> Cancel / Reject</span>
              <span><kbd className="px-1 py-0.5 rounded bg-[#07070e] border border-border text-[10px] font-mono">R</kbd> Read back text</span>
              <span><kbd className="px-1 py-0.5 rounded bg-[#07070e] border border-border text-[10px] font-mono">A</kbd> Accept edit</span>
              <span><kbd className="px-1 py-0.5 rounded bg-[#07070e] border border-border text-[10px] font-mono">Z</kbd> Undo last edit</span>
            </div>
            <p className="text-[10px] text-muted-foreground/60 mt-1">
              Shortcuts work when text input is not focused. Press <kbd className="px-1 py-0.5 rounded bg-[#07070e] border border-border text-[10px] font-mono">?</kbd> to toggle.
            </p>
          </div>
        )}
              ...
              <div className="flex flex-col gap-2 w-full max-w-[300px]">
                {suggestedPrompts.map((prompt, i) => (
                  <button
                    ...
                    className="text-left text-xs px-3 py-2 rounded-lg border border-border bg-[#0c0b18] hover:bg-accent transition-colors"
                  >
                    {prompt}
                  </button>
                ))}
                ...
                if (msg.content === "__loading__") {
                  return (
                    <div key={i} className="flex items-start gap-2.5">
                      ...
                      <div className="rounded-xl bg-[rgba(255,255,255,0.04)] px-3.5 py-2.5 min-w-[140px]">
                        <div className="flex items-center gap-2">
                          <Loader2 className="size-3.5 animate-spin text-amber-500 shrink-0" />
                          <span className="text-xs text-muted-foreground italic">
                            {msg.toolBadges?.some(b => b.status === "pending") ? "Executingâ¦" : "Thinkingâ¦"}
                          </span>
                        </div>
                    ...
                    <div className={cn("max-w-[85%] flex flex-col gap-1")}>
                      <div
                        className={cn(
                          ...
                          msg.role === "user"
                            ...
                            : "bg-[rgba(255,255,255,0.04)] text-foreground"
                        )}
                      >
                        {msg.role === "assistant" ? (
                      ...
                      <div className={cn(
                        ...
                        <button
                          ...
                          className="p-1 rounded text-muted-foreground hover:text-foreground hover:bg-[rgba(255,255,255,0.04)] transition-colors"
                          title="Copy"
                        >
                        ...
                        {msg.role === "assistant" && (
                          <button
                            ...
                            className={cn(
                              ...
                              isSpeaking ? "text-amber-500 hover:text-amber-400" : "text-muted-foreground hover:text-foreground hover:bg-[rgba(255,255,255,0.04)]"
                            )}
        ...
        {showEditPreview && (
          <div className="px-4 py-3 border-t bg-violet-500/5 border-violet-500/20 shrink-0">
            <div className="mb-2">
              ...
              <div className="bg-[#07070e]/60 rounded-lg p-2.5 border border-violet-500/10 max-h-[100px] overflow-y-auto">
            ...
            <div className="flex items-center justify-between">
              <p className="text-[10px] text-muted-foreground">
                <kbd className="px-1 py-0.5 rounded bg-[#07070e] border border-border text-[10px] font-mono">A</kbd> accept{" Â· "}
            ...
            {showHistoryPanel && (
              <div className="max-h-[150px] overflow-y-auto px-4 pb-2 space-y-1.5">
                {[...editHistory].reverse().map((entry) => (
                  <div key={entry.id} className="flex items-center justify-between gap-2 rounded-lg bg-[#07070e]/40 border border-border/50 px-2.5 py-1.5">
        ...
        {attachments.length > 0 && (
          <div className="px-4 py-2 border-t bg-[rgba(255,255,255,0.04)]/30 space-y-1.5 shrink-0">
        ...
        {/* Scroll to bottom â appears when user has scrolled up */}
        {userScrolledUp && (
          <button
            onClick={() => { setUserScrolledUp(false); scrollToBottom(true); }}
            className="absolute bottom-[72px] right-4 z-20 flex items-center justify-center size-7 rounded-full bg-[#07070e] border border-amber-500/40 shadow-md hover:bg-amber-500/10 transition-colors"
        ...
        {/* Input area â wide, comfortable, Titan-style */}
        <div className="border-t bg-[#07070e]/80 backdrop-blur-sm px-3 pt-3 pb-2 sm:rounded-b-2xl shrink-0">
          ...
          <div className={cn(
            ...
            />
            {/* Send button â inside the textarea, bottom-right */}
            <button
              className={cn(
          ...
          {/* Action icon row â below the textarea */}
          <div className="flex items-center justify-between mt-2">
            <div className="flex items-center gap-0.5">
              {/* Attach */}
              <button
                className="p-2 rounded-xl text-muted-foreground hover:text-foreground hover:bg-[rgba(255,255,255,0.04)]/60 transition-all active:scale-95 disabled:opacity-40"
              ...
              {/* Voice edit (pencil) â only when there's text */}
              {hasInputText && voiceState === "idle" && (
                <button
                  ...
                  title="Voice edit â speak to edit (V)"
                  style={{ touchAction: "manipulation", minWidth: 40, minHeight: 40 }}
                >
              ...
              {/* âââ Talk button â tap to speak, AI speaks back âââ */}
              <button
                className={cn(
              ...
              {hasInputText && voiceState === "idle" && !showEditPreview && (
                <button
                  className={cn(
                    ...
                    isSpeaking ? "text-amber-500 bg-amber-500/10" : "text-muted-foreground hover:text-foreground hover:bg-[rgba(255,255,255,0.04)]/60"
                  )}
            ...
            <p className="text-[10px] text-muted-foreground/60 pr-1 hidden sm:block">
              {hasInputText && voiceState === "idle"
                ? "V voice-edit Â· R read back"
                : "V to speak Â· / commands"}
            </p>
