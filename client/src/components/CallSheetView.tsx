...
/**
 * v6.63 — Call Sheet view. Used both inline (CallSheets index) and on the
...
export default function CallSheetView({ data, printMode = false }: Props) {
  ...
  return (
    <div className={printMode ? "bg-white text-black p-8 max-w-[8.5in] mx-auto print:p-4" : "bg-zinc-950 text-zinc-100 p-6 rounded-lg border border-zinc-800"}>
      {/* Header */}
      <div className={`flex items-start justify-between pb-4 mb-4 border-b ${printMode ? "border-zinc-300" : "border-zinc-800"}`}>
        ...
        <div className="text-right">
          ...
          <div className={`text-xs mt-1 ${printMode ? "text-zinc-600" : "text-zinc-400"}`}>
            ...
            {day?.wrapTime ? <> · Wrap: <span className="font-semibold">{day.wrapTime}</span></> : ""}
          </div>
        </div>
      </div>
            ...
            <tbody>
              {scenes.map((s: any, i: number) => (
                <tr key={s.id} className={`border-t ${printMode ? "border-zinc-200" : "border-zinc-800"}`}>
                  ...
                  <td className="px-2 py-1.5">{s.intExt || "—"}</td>
                  <td className="px-2 py-1.5">{s.timeOfDay || "—"}</td>
            ...
            <tbody>
              {crew.map((c: any) => (
                <tr key={c.id} className={`border-t ${printMode ? "border-zinc-200" : "border-zinc-800"}`}>
                  ...
                  <td className="px-2 py-1.5 font-mono">{c.callTimeOverride || day?.callTime || "—"}</td>
                  <td className="px-2 py-1.5">{c.phone || "—"}</td>
                  <td className="px-2 py-1.5 truncate max-w-[180px]">{c.email || "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className={`mt-6 pt-3 border-t text-[10px] ${printMode ? "border-zinc-300 text-zinc-500" : "border-zinc-800 text-zinc-600"} text-center`}>
        Generated by Virelle Studios · {new Date().toLocaleString()}
      </div>
    </div>
  );
}
