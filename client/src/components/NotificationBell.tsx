...
export default function NotificationBell() {
  ...
  return (
    <div className="relative flex items-center gap-1" ref={panelRef}>
      <button
        onClick={() => { window.location.href = "/virelle-broadcast-render?workspace=adult"; }}
        className="flex h-9 items-center gap-1.5 rounded-lg border border-fuchsia-500/30 bg-fuchsia-500/10 px-2 text-xs font-medium text-fuchsia-200 transition-colors hover:bg-fuchsia-500/20"
        title="Adult Workspace — separate 18+ registration and tools"
        aria-label="Open Adult Workspace"
      >
        <LockKeyhole className="h-4 w-4" />
        <span className="hidden xl:inline">Adult 18+</span>
      </button>
      ...
      {open && (
        <div className="absolute right-0 top-full z-[100] mt-2 flex max-h-[480px] w-80 flex-col overflow-hidden rounded-xl border border-white/10 bg-[#0c0b18] shadow-2xl sm:w-96">
          <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
            <h3 className="text-sm font-semibold gradient-text-gold">Notifications</h3>
