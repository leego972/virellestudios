...
export default function VirelleChatBubble({
  ...
  return (
    <div className="fixed bottom-0 right-0 sm:bottom-6 sm:right-6 z-50 w-full sm:w-[380px] sm:max-w-[calc(100vw-2rem)]">
      <div className="bg-[#0c0b18] border border-border sm:rounded-xl shadow-2xl shadow-black/20 overflow-hidden flex flex-col" style={{ maxHeight: "min(600px, calc(100dvh - env(safe-area-inset-top, 0px) - 1rem))" }}>
        {/* Header */}
          ...
          {chatHistory.map((msg, idx) => (
            <div key={idx} className={`flex gap-2.5 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
              ...
              <div className={`rounded-lg px-3 py-2 max-w-[85%] ${
                msg.role === "user"
                  ? "bg-amber-500 text-primary-foreground rounded-tr-none"
