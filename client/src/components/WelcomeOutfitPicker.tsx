...
export default function WelcomeOutfitPicker() {
  ...
  const claimMut = (trpc as any).lamaloGifts?.claimGift?.useMutation?.({
    onSuccess: () => toast.success("Your 2 free Lamalo virtual outfits are now in your wardrobe inventory."),
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center gap-2 mb-1">
            ...
            <DialogTitle>Welcome Gift - 2 Free Lamalo Virtual Outfits</DialogTitle>
          </div>
          <DialogDescription>
            Choose 2 virtual outfits from Lamalo Fashions as a welcome gift for your first characters.
            Lamalo Fashions is VirElle's virtual-only wardrobe line: no physical product is shipped, and the selected items stay permanently in your Virelle inventory for AI film use.
          </DialogDescription>
        </DialogHeader>

        <div className="rounded-lg border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-muted-foreground">
          Virtual item only. Permanent inventory access. No rental, no lease, no expiry.
        </div>
          ...
          {outfits.map((item: any) => {
            ...
            return (
              <Card
                ...
                className={`cursor-pointer transition-all relative ${disabled ? "opacity-40" : ""} ${isSelected ? "ring-2 ring-primary shadow-md" : "hover:shadow-sm"}`}
              >
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t">
          <p className="text-sm text-muted-foreground">
            {selected.length}/2 virtual outfits selected
          </p>
