...
export default function DayOfDaysGrid({ scenes, days, characters }: Props) {
  ...
  function cellLabel(cid: number, dayId: number): { label: string; color: string } {
    ...
    return { label: "·", color: "bg-zinc-800 text-zinc-200" };
  }
  ...
  return (
    <div className="space-y-3">
      <div className="overflow-x-auto border border-zinc-800 rounded">
        <table className="text-xs">
          <thead className="bg-zinc-900/80 text-zinc-400">
            <tr>
              <th className="sticky left-0 bg-zinc-900/80 px-3 py-2 text-left font-semibold border-r border-zinc-800 min-w-[140px]">Cast</th>
              {sortedDays.map((d) => (
                <th key={d.id} className="px-2 py-2 text-center min-w-[42px] border-r border-zinc-800/50">
              ...
              <th className="px-2 py-2 text-center bg-zinc-900 border-l border-zinc-700 min-w-[50px]">Work</th>
              <th className="px-2 py-2 text-center bg-zinc-900 min-w-[50px]">Hold</th>
            </tr>
          </thead>
          <tbody>
            {characters.map((c) => {
              ...
              return (
                <tr key={c.id} className="border-t border-zinc-800 hover:bg-zinc-900/40">
                  <td className="sticky left-0 bg-zinc-950 px-3 py-1.5 text-zinc-200 font-medium border-r border-zinc-800">{c.name}</td>
                  {sortedDays.map((d) => {
                    const r = cellLabel(c.id, d.id);
                    return (
                      <td key={d.id} className="px-1 py-0 text-center border-r border-zinc-800/50">
      ...
      <div className="text-[11px] text-zinc-500 flex flex-wrap gap-3">
        ...
        <span><span className="bg-zinc-800 text-zinc-200 px-1.5 rounded">·</span> Working</span>
