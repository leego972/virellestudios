...
export default defineConfig(async ({ command }) => {
  ...
  // Use dynamic imports so esbuild does NOT bundle them into dist/index.js
  // (they are devDependencies, not available in production).
  const isDevServer = command === "serve";

  const extraDevPlugins: Plugin[] = [];
  if (isDevServer) {
    ...
    extraDevPlugins.push(jsxLocPlugin(), vitePluginManusRuntime());
  }
