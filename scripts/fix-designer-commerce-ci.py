...
text = replace_once(
    ...
    "profile form record typing",
)
write(path, text)

# Client address state callback typing.
...
# Stripe SDK version compatibility and fulfilment result property.
...
text = replace_once(
    ...
    "Stripe payment intent typing",
)
text = replace_once(
    text,
    "        copies: fulfilled.copies,\n",
    "        copies: fulfilled.inventoryCopies,\n",
    "fulfilment inventory property",
)
write(path, text)

# Add an idempotent rollback for signup failures.
...
if "rollbackNewUserRegistration" not in text:
    if anchor not in text:
        raise RuntimeError("Missing rollback insertion anchor")
    text = text.replace(anchor, addition + anchor, 1)
write(path, text)

# Use the rollback helper from the large app router without adding a DB API that
# does not exist.
