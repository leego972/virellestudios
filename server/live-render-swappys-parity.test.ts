...
test("Render serves the merged Swappys and downloadable-app parity release", async () => {
  let deployed = false;
  ...
  expect(deployed, `Render did not expose the merged client bundle. Last error: ${String(lastError || "none")}`).toBe(true);
