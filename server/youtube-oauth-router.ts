...
import { parse as parseCookies } from "cookie";
  ...
  export function registerYouTubeOAuthRoutes(app: Express) {
    ...
    app.get("/api/youtube/callback", requireAdminExpress, async (req: Request, res: Response) => {
      ...
      if (!code) {
        ...
        const _expectedState = _cookies["__virelle_oauth_state_youtube"];
        ...
        if (!_expectedState || !_receivedState || _expectedState !== _receivedState) {
          logger.warn("[Youtube OAuth] State mismatch — possible CSRF", { expected: _expectedState?.slice(0,8), received: _receivedState?.slice(0,8) });
  ...
  <body>
    <div class="card">
      ...
      <p class="sub">Copy this token and add it to Railway to enable auto-posting.</p>
      ...
      <div class="steps">
        ...
        <div class="step"><strong>Step 4 —</strong> Redeploy. YouTube auto-posting is live 🎬</div>
      </div>
