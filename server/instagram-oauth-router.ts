...
import { parse as parseCookies } from "cookie";
  ...
  export function registerInstagramOAuthRoutes(app: Express) {
    ...
    app.get("/api/instagram/connect", requireAdminExpress, (req: Request, res: Response) => {
      ...
      res.cookie("__virelle_oauth_state_instagram", oauthState, {
        httpOnly: true,
        sameSite: "lax",
    ...
    app.get("/api/instagram/callback", requireAdminExpress, async (req: Request, res: Response) => {
      ...
      if (!code) {
        ...
        const _expectedState = _cookies["__virelle_oauth_state_instagram"];
        const _receivedState = req.query.state as string;
        res.clearCookie("__virelle_oauth_state_instagram");
        if (!_expectedState || !_receivedState || _expectedState !== _receivedState) {
          logger.warn("[Instagram OAuth] State mismatch — possible CSRF", { expected: _expectedState?.slice(0,8), received: _receivedState?.slice(0,8) });
  ...
  <body>
    <div class="card">
      ...
      <a class="back" href="/content-creator">← Back to Content Creator</a>
        ...
        logger.error("[Instagram OAuth] Callback error", { error: String(err) });
