  ...
  project: router({
    ...
    create: creationProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // v6.90 — One-button demo short: creates project + 5 scenes + fires video generation automatically
        createDemoShort: protectedProcedure
          .mutation(async ({ ctx }) => {
            ...
            // 2 — Create 5 pre-written cinematic scenes
            const DEMO_SCENES = [
            ...
            // 3 — Mark project as generating so the UI shows live progress
            await db.updateProject(project.id, ctx.user.id, { status: "generating", progress: 0 });

            // 4 — Fire video generation for all 5 scenes in background (non-blocking, same pattern as quickGenerate)
            ...
            setImmediate(async () => {
              try {
                ...
                await Promise.allSettled(
                  createdScenes.map(async (scene: any, idx: number) => {
                    try {
                      ...
                      const isAsync =
                        videoResult.videoUrl.startsWith("runway-pending:") ||
                        videoResult.videoUrl.startsWith("fal-pending") ||
                        videoResult.videoUrl.startsWith("veo3-pending:");
                      ...
                      if (isAsync && videoResult.jobId) {
                        await db.createGenerationJob({
                          ...
                          metadata: JSON.stringify({
                            runwayTaskId: videoResult.videoUrl.startsWith("runway-pending:") ? videoResult.jobId : undefined,
                            falRequestId: videoResult.videoUrl.startsWith("fal-pending") ? videoResult.jobId : undefined,
                        ...
                        if (idx === 0 && videoResult.thumbnailUrl) {
                          await db.updateProject(projectId, userId, { thumbnailUrl: videoResult.thumbnailUrl }).catch(() => {});
                        }
                      }
                    } catch (sceneErr: any) {
                      console.error(`[DemoShort] Scene ${idx + 1} generation failed:`, sceneErr.message);
                      await db.updateScene(scene.id, { status: "failed" }).catch(() => {});
                    }
                  })
                );

                // Mark project complete once all scenes resolve
                ...
                if (allDone) {
                  await db.updateProject(projectId, userId, { status: "completed", progress: 100 }).catch(() => {});
                }
              } catch (err: any) {
                console.error("[DemoShort] Background generation error:", err.message);
