/**
   ...
   *
   * End-to-end automated video ad generation + distribution.
   * Video cascade: Pollinations (free built-in) → Runway ML → fal.ai → Replicate
   * Distribution: TikTok (portrait 9:16) + YouTube (landscape 16:9)
   *
   * Required Railway env vars to enable distribution:
   *   TikTok  : TIKTOK_CREATOR_TOKEN  +  TIKTOK_ACCESS_TOKEN
   *   YouTube : YOUTUBE_CLIENT_ID  +  YOUTUBE_CLIENT_SECRET  +  YOUTUBE_REFRESH_TOKEN
   *   (Video generation works NOW with zero extra keys — Pollinations has built-in fallback)
   */
  ...
  export const AD_THEMES = [
    ...
    { theme: "Script to Screen in Minutes",    angle: "Type your story — watch it become a film",                          cta: "Try it free at virelle.life" },
  ...
  async function generateAdScript(
    ...
    const duration  = isTikTok ? "15–30 seconds (3 scenes × ~8s each)" : "30–45 seconds (4 scenes × ~10s each)";
    const format    = isTikTok ? "portrait 9:16, mobile-first"         : "landscape 16:9, widescreen cinematic";
    ...
    const sysMsg = [
      ...
      "Each scene needs: (1) a detailed AI video generation prompt with camera movement + lighting, (2) brief voiceover copy.",
      "The visual prompts will be fed directly into Runway/Pollinations AI — be specific and cinematic.",
            ...
            properties: {
              ...
              scenes: {
                ...
                items: {
                  ...
                  required: ["visualPrompt", "voiceover", "durationSeconds"],
  ...
  async function generateAdVideo(
    ...
    try {
      ...
      const result = await byokGenerateVideo(keys, {
        prompt,
  ...
  export async function generateAndPostVideoAd(
      ...
      try {
        ...
        if (platform === "tiktok") {
          ...
          if (videoUrl) {
            ...
            results.push({ status: postResult.success ? "posted" : "failed", platform: "tiktok", videoUrl,
              ...
              details: postResult.success ? "TikTok video posted (publishId: " + postResult.publishId + ")"
                                          : "TikTok post failed: " + postResult.error });
          } else {
            log.info("[VideoAdPipeline] Generating TikTok photo carousel fallback...");
            const carouselUrls = await generateCarouselImages({
              title: script.headline, description: caption, hashtags: script.hashtags,
              hook: script.hook, visualStyle: "cinematic dark premium",
            ...
            if (carouselUrls.length > 0) {
              ...
              results.push({ status: photoResult.success ? "posted" : "failed", platform: "tiktok_carousel", script,
                details: photoResult.success ? "TikTok photo carousel posted (" + carouselUrls.length + " images)"
                                             : "TikTok carousel failed: " + photoResult.error });
            } else {
        ...
        // ── YouTube ────────────────────────────────────────────────────────────
          ...
          const ytDesc = script.scenes.map((s, i) => "[" + (i + 1) + "] " + s.voiceover).join("\n\n")
            + "\n\n" + script.cta + "\n\nvirelle.life — AI Filmmaking Platform\n\n" + hashtagStr;
          const ytResult = await uploadVideoToYouTube({
            videoUrl, title: script.headline.slice(0, 100), description: ytDesc,
            tags: script.hashtags, privacyStatus: "public", categoryId: "24",
          });
    ...
    log.info({ count: results.length, statuses: results.map(r => r.platform + ":" + r.status) },
      "[VideoAdPipeline] Campaign complete");
    return results;
  }
  ...
  async function saveAdToDb(platform: string, script: VideoAdScript,
    ...
    try {
      ...
      await db.insert(marketingContent).values({
        ...
        metadata: {
          hook: script.hook, cta: script.cta, hashtags: script.hashtags,
          videoUrl: videoUrl ?? null, sceneCount: script.scenes.length,
          type: "video_ad", pipelineVersion: "2.0",
        },
      } as any);
    } catch (err) {
