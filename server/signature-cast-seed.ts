/**
   * signature-cast-seed.ts
   *
   * Seeds the Virelle Signature Cast (10 AI actors) into the characters table
   * as global library characters (projectId = NULL, userId = admin user id).
   *
   * Uses Drizzle db.execute(sql`...`) with INSERT IGNORE — safe to re-run.
   */
  ...
  async function insertCast(userId: number, cast: CastMember) {
    ...
    await db.execute(sql`
      ...
      VALUES
        (${userId}, NULL, ${cast.name}, ${cast.role}, ${cast.storyImportance},
         ${cast.screenTime}, ${cast.description}, ${cast.nationality},
         ${cast.castingNotes}, ${cast.isAiActor ? 1 : 0}, ${cast.aiActorId})
    `);
  }

  export async function runSignatureCastSeed(userId: number) {
    for (const cast of SIGNATURE_CAST) {
      await insertCast(userId, cast);
    }
  }
