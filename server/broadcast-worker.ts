...
async function initiateBroadcastSession(dbConn: any, job: any): Promise<void> {
  ...
  try {
    ...
    const recordingUrl = bridge.recordingUrl || bridge.outputUrl || null;
    await dbConn.execute(sql`
      UPDATE virelle_video_transform_jobs
      SET status='processing', providerJobId=${bridge.sessionId},
          outputVideoUrl=${recordingUrl}, previewImageUrl=${bridge.previewUrl || null},
          errorMessage=NULL, updatedAt=NOW()
      WHERE id=${jobId}
    `);
    logger.info(
      `[BroadcastWorker] Session ${jobId} submitted; outputs=${channels.length}, recording=${recordingUrl ? "available" : "pending"}`,
    );
  } catch (error: any) {
    ...
    const rows: any = await dbConn.execute(sql`
      ...
      WHERE mode='broadcast'
        AND (
          ...
          OR (status='waiting_for_provider'
