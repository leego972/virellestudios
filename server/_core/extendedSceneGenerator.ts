    ...
    if (status === "SUCCEEDED" || status === "succeeded") {
      ...
      if (videoUrl && typeof videoUrl === "string") {
        ...
        try {
          ...
          logger.warn(`[ExtendedScene] Storage unavailable for Runway video (${storageErr.message}), using raw CDN URL`);
          url = videoUrl;
        }
    ...
    if (resp.ok) {
      ...
      if (data.done) {
        ...
        try {
          ...
          logger.warn(`[ExtendedScene] Storage unavailable for Veo3 video (${storageErr.message}), using raw CDN URL`);
          s3Url = downloadUrl;
        }
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try { return await fn(); } catch (err: any) {
      ...
      if (attempt < maxRetries) {
        ...
        logger.warn(`[ExtendedScene] Sub-clip attempt ${attempt + 1} failed (${err.message}). Retrying in ${delay}ms...`);
...
async function extractLastFrame(videoUrl: string, projectId: number, sceneId: number): Promise<string | undefined> {
  ...
  try {
    ...
    try {
      ...
      logger.warn(`[ExtendedScene] Storage unavailable for last frame (${storageErr.message}), skipping frame upload`);
      return undefined;
    }
  } catch (err) {
...
async function extractFirstFrame(videoUrl: string, projectId: number, sceneId: number): Promise<string | undefined> {
  ...
  try {
    ...
    try {
      ...
      logger.warn(`[ExtendedScene] Storage unavailable for thumbnail (${storageErr.message}), skipping thumbnail upload`);
      return undefined;
    }
  } catch {
    return undefined;
...
async function stitchSubClips(
  ...
  if (clipUrls.length === 1) {
    ...
    try {
      ...
      logger.warn(`[ExtendedScene] Failed to re-upload single clip to S3, using CDN URL as fallback: ${uploadErr.message}`);
    }
  ...
  try {
    ...
    for (let i = 0; i < clipUrls.length; i++) {
      ...
      try {
        ...
        logger.warn(`[ExtendedScene] Failed to download clip ${i + 1} for stitching: ${dlErr.message}`);
      }
    }
    ...
    try {
      ...
      logger.warn(`[ExtendedScene] Storage unavailable for stitched video (${storageErr.message}), using first clip URL as fallback`);
      videoUrl = clipUrls[0];
    }
...
export async function generateExtendedScene(
    ...
    try {
      ...
      if (!effectiveImageUrl && activeProvider === "runway") {
        try {
          ...
          logger.warn(`[ExtendedScene] Keyframe auto-gen failed for sub-clip ${i + 1}: ${kfErr.message}`);
        }
      }
      ...
      logger.error(`[ExtendedScene] Sub-clip ${i + 1} failed:`, err.message);
      ...
      try {
        ...
        if (fbCheck.ok) {
          ...
          logger.warn(`[ExtendedScene] Sub-clip ${i + 1} replaced with Pollinations still-frame fallback`);
        }
      } catch {
  ...
  if (generatedClipUrls.length > 0 && generatedClipUrls.length < subShots.length) {
    logger.warn(`[ExtendedScene] Scene ${request.sceneId}: partial render â ${generatedClipUrls.length}/${subShots.length} clips succeeded`);
  }
