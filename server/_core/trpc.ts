...
import {
  swappysRateLimitSubject,
  validateSwappysImageDataUrl,
} from "./swappysValidation";
import {
  assertSwappysCreativePolicy,
  swappysCreativePromptDirective,
  type SwappysContentMode,
} from "./swappysPolicy";
import { screenContentRequest } from "./contentCompliance";
import {
  getUserPortal,
...
const requireUser = t.middleware(async opts => {
  ...
  if ((ctx.user as any).isFrozen) {
    throw new TRPCError({
      code: "FORBIDDEN",
      message: (ctx.user as any).frozenReason
        || "This account has been deactivated following a confirmed policy violation.",
    });
  }
...
const guardStudioSwappys = t.middleware(async (opts) => {
  if (
    ...
    throw new TRPCError({
      ...
      message: "BROADCAST_BRIDGE_NOT_CONFIGURED: Live broadcasting is unavailable until BROADCAST_BRIDGE_URL and BROADCAST_BRIDGE_TOKEN are configured. No credits were charged.",
    });
  }
  ...
  const publicFigureLikeness = input.publicFigureLikeness === true
    || operations.includes("public-figure-likeness");
  const aiGeneratedCharactersOnly = input.aiGeneratedCharactersOnly === true
    || operations.includes("ai-generated-characters-only");
  const policyText = [
  ...
  if (opts.path.endsWith("createSwappysDigitalDoubleJob")) {
    ...
    if (contentMode === "open_adult") {
      input.operations = Array.from(new Set([
        ...
        ...(aiGeneratedCharactersOnly ? ["ai-generated-characters-only"] : []),
      ])).slice(0, 32);
    }
  }
