...
export type WorkspaceKind = "standard" | "adult";
export type MediaKind = "video" | "broadcast";
...
export type AdultProfileInput = {
  ...
  soleResponsibilityAccepted: boolean;
  consentPolicyAccepted: boolean;
  retentionPolicyAccepted: boolean;
};

export type ContentScreenInput = {
  userId: number;
...
const TERMS_VERSION = "adult-workspace-2026-07";
...
function archiveMaxBytes(): number {
  ...
  return 2 * 1024 * 1024 * 1024; // 2 GB streaming ceiling
}
...
function sourceKeyFromOwnStorage(url: string, config: ReturnType<typeof storageConfig>): string | null {
...
function archiveKey(candidate: ArchiveCandidate): string {
  ...
  const extension = (() => {
    try {
      ...
      const match = pathname.match(/\.(mp4|mov|webm|mkv|avi|m4v)$/i);
      if (match) return `.${match[1].toLowerCase()}`;
    } catch {
      // use MIME fallback
    }
    if (candidate.mimeType?.includes("webm")) return ".webm";
    if (candidate.mimeType?.includes("quicktime")) return ".mov";
    return ".mp4";
  })();
  return [
    ARCHIVE_PREFIX,
...
export async function ensureComplianceTables(dbConn?: any): Promise<any> {
  ...
  await connection.execute(sql`
    CREATE TABLE IF NOT EXISTS adult_workspace_profiles (
      id INT AUTO_INCREMENT PRIMARY KEY,
      userId INT NOT NULL UNIQUE,
      legalFirstName VARCHAR(120) NOT NULL,
      legalLastName VARCHAR(120) NOT NULL,
      dateOfBirth DATE NOT NULL,
      residentialAddress1 VARCHAR(255) NOT NULL,
      residentialAddress2 VARCHAR(255) NULL,
      city VARCHAR(120) NOT NULL,
      stateRegion VARCHAR(120) NOT NULL,
      postalCode VARCHAR(32) NOT NULL,
      country VARCHAR(120) NOT NULL,
      phone VARCHAR(40) NOT NULL,
      verificationStatus VARCHAR(32) NOT NULL DEFAULT 'pending_review',
      adultAttestationAccepted TINYINT(1) NOT NULL DEFAULT 0,
      soleResponsibilityAccepted TINYINT(1) NOT NULL DEFAULT 0,
      consentPolicyAccepted TINYINT(1) NOT NULL DEFAULT 0,
      retentionPolicyAccepted TINYINT(1) NOT NULL DEFAULT 0,
      ...
      INDEX idx_awp_status (verificationStatus)
  ...
  await connection.execute(sql`
    CREATE TABLE IF NOT EXISTS compliance_media_archive (
      ...
      createdAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updatedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      INDEX idx_cma_user (userId),
      INDEX idx_cma_status (archiveStatus),
      INDEX idx_cma_retention (retainedUntil, legalHold),
      INDEX idx_cma_workspace (workspace),
      INDEX idx_cma_started (startedAt)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
  `);
  ...
  await connection.execute(sql`
    CREATE TABLE IF NOT EXISTS blacklisted_users (
      ...
      INDEX idx_bu_incident (incidentId)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
  `);
  ...
  await connection.execute(sql`
    CREATE TABLE IF NOT EXISTS compliance_access_log (
      ...
      createdAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      INDEX idx_cal_admin (adminUserId),
      INDEX idx_cal_archive (archiveId),
      INDEX idx_cal_incident (incidentId),
...
export async function getAdultWorkspaceProfile(userId: number) {
  const connection = await ensureComplianceTables();
  const result = await connection.execute(sql`
    SELECT id, userId, legalFirstName, legalLastName, dateOfBirth,
           residentialAddress1, residentialAddress2, city, stateRegion,
           postalCode, country, phone, verificationStatus,
           adultAttestationAccepted, soleResponsibilityAccepted,
           consentPolicyAccepted, retentionPolicyAccepted, termsVersion,
           submittedAt, reviewedAt, reviewNotes, createdAt, updatedAt
    FROM adult_workspace_profiles WHERE userId=${userId} LIMIT 1
  `);
  return rowsFrom(result)[0] || null;
}

export async function submitAdultWorkspaceProfile(userId: number, input: AdultProfileInput) {
  if (calculateAge(input.dateOfBirth) < 18) {
    throw new TRPCError({
      code: "FORBIDDEN",
      message: "The Adult Workspace is available only to individuals aged 18 or older.",
    });
  }
  if (
    !input.adultAttestationAccepted ||
    !input.soleResponsibilityAccepted ||
    !input.consentPolicyAccepted ||
    !input.retentionPolicyAccepted
  ) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "All Adult Workspace legal attestations must be accepted.",
    });
  }

  const connection = await ensureComplianceTables();
  await connection.execute(sql`
    INSERT INTO adult_workspace_profiles
      (userId, legalFirstName, legalLastName, dateOfBirth,
       residentialAddress1, residentialAddress2, city, stateRegion,
       postalCode, country, phone, verificationStatus,
       adultAttestationAccepted, soleResponsibilityAccepted,
       consentPolicyAccepted, retentionPolicyAccepted, termsVersion,
    ...
    VALUES
      (${userId}, ${input.legalFirstName.trim()}, ${input.legalLastName.trim()}, ${input.dateOfBirth},
       ${input.residentialAddress1.trim()}, ${input.residentialAddress2?.trim() || null},
       ${input.city.trim()}, ${input.stateRegion.trim()}, ${input.postalCode.trim()},
       ${input.country.trim()}, ${input.phone.trim()}, 'pending_review',
       1, 1, 1, 1, ${TERMS_VERSION}, NOW(), NULL, NULL, NULL)
    ON DUPLICATE KEY UPDATE
      legalFirstName=VALUES(legalFirstName), legalLastName=VALUES(legalLastName),
      dateOfBirth=VALUES(dateOfBirth), residentialAddress1=VALUES(residentialAddress1),
      residentialAddress2=VALUES(residentialAddress2), city=VALUES(city),
      stateRegion=VALUES(stateRegion), postalCode=VALUES(postalCode),
      country=VALUES(country), phone=VALUES(phone),
      verificationStatus='pending_review', adultAttestationAccepted=1,
      soleResponsibilityAccepted=1, consentPolicyAccepted=1,
      retentionPolicyAccepted=1, termsVersion=VALUES(termsVersion),
      submittedAt=NOW(), reviewedAt=NULL, reviewedBy=NULL, reviewNotes=NULL
  `);
  await connection.execute(sql`
    UPDATE users SET isAdultVerified=0, updatedAt=NOW() WHERE id=${userId}
  `);
  return getAdultWorkspaceProfile(userId);
}

export async function hasApprovedAdultWorkspace(userId: number): Promise<boolean> {
  const connection = await ensureComplianceTables();
  const result = await connection.execute(sql`
    SELECT p.userId
    FROM adult_workspace_profiles p
    INNER JOIN users u ON u.id=p.userId
    WHERE p.userId=${userId}
      AND p.verificationStatus='approved'
      AND p.adultAttestationAccepted=1
      AND p.soleResponsibilityAccepted=1
      AND p.consentPolicyAccepted=1
      AND p.retentionPolicyAccepted=1
      AND u.isAdultVerified=1
      AND u.isFrozen=0
    LIMIT 1
  `);
  return Boolean(rowsFrom(result)[0]);
}

export async function assertAdultWorkspaceAccess(userId: number): Promise<void> {
  if (!(await hasApprovedAdultWorkspace(userId))) {
    throw new TRPCError({
      code: "FORBIDDEN",
      message: "ADULT_WORKSPACE_APPROVAL_REQUIRED: Submit your individual 18+ profile and wait for administrator approval.",
    });
  }
}

export async function listAdultProfileReviews(limit = 100) {
  const connection = await ensureComplianceTables();
  const result = await connection.execute(sql`
    SELECT p.*, u.name AS accountName, u.email, u.createdAt AS accountCreatedAt
    FROM adult_workspace_profiles p
    INNER JOIN users u ON u.id=p.userId
    ORDER BY FIELD(p.verificationStatus, 'pending_review', 'rejected', 'approved'), p.submittedAt DESC
    LIMIT ${Math.max(1, Math.min(limit, 500))}
  `);
  return rowsFrom(result);
}

export async function reviewAdultProfile(params: {
  adminUserId: number;
  userId: number;
  ...
  const connection = await ensureComplianceTables();
  const profile = await getAdultWorkspaceProfile(params.userId);
  if (!profile) throw new TRPCError({ code: "NOT_FOUND", message: "Adult profile not found." });
  if (params.decision === "approved" && calculateAge(String(profile.dateOfBirth).slice(0, 10)) < 18) {
    throw new TRPCError({ code: "BAD_REQUEST", message: "The submitted date of birth is under 18." });
  }

  await connection.execute(sql`
    UPDATE adult_workspace_profiles
    SET verificationStatus=${params.decision}, reviewedAt=NOW(),
        reviewedBy=${params.adminUserId}, reviewNotes=${params.notes || null}
    WHERE userId=${params.userId}
  `);
  await connection.execute(sql`
    UPDATE users SET isAdultVerified=${params.decision === "approved" ? 1 : 0}, updatedAt=NOW()
    WHERE id=${params.userId}
  `);
  await logComplianceAccess({
    adminUserId: params.adminUserId,
    action: `adult_profile_${params.decision}`,
    targetUserId: params.userId,
    metadata: { notes: params.notes || null },
    ipAddress: params.ipAddress,
  });
  return getAdultWorkspaceProfile(params.userId);
}
  ...
  const result: any = await connection.execute(sql`
    ...
    VALUES
      (${params.userId}, ${params.archiveId || null}, ${params.sourceType},
       ...
       ${params.evidenceArchiveKey || null}, ${JSON.stringify(params.classifierSignals || {})}, 1)
  `);
...
export async function screenContentRequest(input: ContentScreenInput): Promise<void> {
  const rawText = String(input.text || "").slice(0, 12_000);
  const text = stripSafetyNegations(rawText);
  ...
  if (hasNonConsensual) {
    ...
    severity = "critical";
  } else if ((hasMinor || targetUnder18 || hasTeen) && hasExplicit) {
    category = "suspected_csam_request";
    severity = "critical";
  } else if ((hasMinor || targetUnder18 || hasTeen) && hasSexualised) {
    category = "suspected_minor_sexualisation";
    severity = "critical";
  } else if (input.workspace === "adult" && (hasMinor || hasTeen || targetUnder18)) {
  ...
  } else if (input.workspace === "adult" && !input.consentConfirmed && !input.aiGeneratedCharactersOnly) {
  ...
  throw new TRPCError({
    code: "FORBIDDEN",
    message: `CONTENT_BLOCKED_PENDING_REVIEW: This request was blocked and sent for administrator review (incident #${incidentId}). No account action has been taken.`,
  });
}

export async function registerArchiveCandidate(candidate: ArchiveCandidate): Promise<number | null> {
  ...
  const retainedUntil = addDays(candidate.completedAt || candidate.startedAt, retentionDays());
  const result: any = await connection.execute(sql`
  ...
  const existing = await connection.execute(sql`
    SELECT id FROM compliance_media_archive WHERE sourceFingerprint=${sourceFingerprint} LIMIT 1
  `);
...
async function copyCandidateToPrivateArchive(row: any): Promise<string> {
  ...
  if (!config) {
    throw new Error(
      "Compliance archive storage is not configured. Set AWS credentials and COMPLIANCE_ARCHIVE_BUCKET (recommended).",
    );
  }
...
export async function processPendingArchives(limit = 3): Promise<number> {
  ...
  `);
  const records = rowsFrom(result);
  let archived = 0;
  for (const row of records) {
    const id = Number(row.id);
    await connection.execute(sql`
      UPDATE compliance_media_archive SET archiveStatus='copying', archiveError=NULL WHERE id=${id}
    `);
    try {
      const key = await copyCandidateToPrivateArchive(row);
      await connection.execute(sql`
        UPDATE compliance_media_archive
        SET archiveStatus='archived', archiveObjectKey=${key}, archiveError=NULL, archivedAt=NOW()
        WHERE id=${id}
      `);
      archived++;
    } catch (error: any) {
...
export async function scanSitewideVideoOutputs(): Promise<number> {
  ...
  const registerRows = async (query: any, mapper: (row: any) => ArchiveCandidate) => {
    try {
  ...
  await registerRows(sql`
    ...
    WHERE j.status='completed' AND j.resultUrl IS NOT NULL AND j.resultUrl <> '' AND a.id IS NULL
...
export async function purgeExpiredArchives(limit = 50): Promise<number> {
  ...
  for (const row of rowsFrom(result)) {
    try {
      ...
      await connection.execute(sql`
        UPDATE compliance_media_archive
        SET archiveStatus='expired_deleted', archiveObjectKey=NULL, expiredDeletedAt=NOW()
...
export async function listComplianceArchive(params: {
  ...
  const connection = await ensureComplianceTables();
  const workspace = params.workspace && params.workspace !== "all" ? params.workspace : null;
  const limit = Math.max(1, Math.min(params.limit || 100, 500));
  const result = await connection.execute(sql`
    SELECT a.id, a.userId, a.accountName, u.email, a.sourceType, a.sourceTable,
           a.sourceId, a.workspace, a.mediaKind, a.userDownloadUrl,
           a.archiveStatus, a.archiveError, a.mimeType, a.startedAt,
           a.completedAt, a.retainedUntil, a.legalHold, a.legalHoldReason,
...
export async function getAdminArchiveDownloadUrl(params: {
  ...
  if (row.archiveStatus !== "archived" || !row.archiveObjectKey) {
    throw new TRPCError({ code: "PRECONDITION_FAILED", message: "Private archive copy is not available." });
  }
  const config = storageConfig();
  if (!config) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Archive storage is unavailable." });
...
export async function listModerationIncidents(params: {
  ...
  const result = await connection.execute(sql`
    SELECT i.*, u.name AS accountName, u.email, u.phone, u.country, u.city,
           a.accountName AS archivedAccountName, a.mediaKind, a.startedAt,
           a.retainedUntil, a.archiveStatus
...
export async function confirmModerationViolation(params: {
  ...
  await connection.execute(sql`
    UPDATE adult_workspace_profiles
    SET verificationStatus='revoked', reviewedAt=NOW(), reviewedBy=${params.adminUserId},
        reviewNotes=CONCAT('Revoked after confirmed incident #', ${params.incidentId})
    WHERE userId=${userId}
  `);
  await connection.execute(sql`
    UPDATE compliance_media_archive
