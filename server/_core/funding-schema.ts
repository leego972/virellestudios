...
export async function ensureFundingCommandSchema(db: Db): Promise<void> {
  if (!schemaPromise) {
    schemaPromise = (async () => {
      const statements = [
        sql`CREATE TABLE IF NOT EXISTS funding_source_metadata (
        ...
        sql`CREATE TABLE IF NOT EXISTS funding_profiles (
        ...
        sql`CREATE TABLE IF NOT EXISTS funding_drafts (
          id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
          userId INT NOT NULL,
          projectId INT NOT NULL,
        ...
        sql`CREATE TABLE IF NOT EXISTS funding_shortlists (
          id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
          userId INT NOT NULL,
          projectId INT NOT NULL,
          fundingSourceId INT NOT NULL,
        ...
        sql`CREATE TABLE IF NOT EXISTS funding_applications (
          id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
          userId INT NOT NULL,
          projectId INT NULL,
          fundingSourceId INT NOT NULL,
        ...
        sql`CREATE TABLE IF NOT EXISTS funding_application_events (
          id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
        ...
        sql`CREATE TABLE IF NOT EXISTS funding_listing_reports (
          id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
          fundingSourceId INT NOT NULL,
          userId INT NOT NULL,
        ...
        sql`CREATE TABLE IF NOT EXISTS funding_reminder_log (
          id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
          userId INT NOT NULL,
      ...
      await db.execute(sql`INSERT IGNORE INTO funding_source_metadata (
        fundingSourceId, sourceCategory, listingStatus, verificationStatus, applicationOpen, confidence
      ) SELECT
        id,
        CASE
          WHEN LOWER(CONCAT_WS(' ', organization, type, fundingForm, supports)) REGEXP 'crowd|kickstarter|indiegogo|seed.?spark|pozible|fundrazr' THEN 'crowdfunding'
          WHEN LOWER(CONCAT_WS(' ', organization, type, fundingForm, supports)) REGEXP 'tax|rebate|offset|incentive|credit' THEN 'incentive'
          WHEN LOWER(CONCAT_WS(' ', organization, type, fundingForm, supports)) REGEXP 'market|lab|pitch|forum' THEN 'market'
          WHEN LOWER(CONCAT_WS(' ', organization, type, fundingForm, supports)) REGEXP 'grant|fund|public agency|screen agency|film commission' THEN 'grant'
          ELSE 'finance_pathway'
        END,
        CASE WHEN organization IN (
          'Production Company Deals', 'Talent Agency Financing', 'Netflix Production Deals',
          'Amazon Studios Financing', 'Hulu Originals Program', 'Apple TV+ Originals'
        ) THEN 'industry_reference' ELSE 'opportunity' END,
        'needs_review', 'unknown', 'unverified'
