...
export const LAMALO_BRAND_NAME = "Lamalo Fashions";
...
function itemIssues(item: any): string[] {
  ...
  if (!usableImage(item.primaryImageUrl) && images.length === 0) {
    issues.push("Item has no production-ready HTTP image; local/placeholder catalogue images are not accepted.");
  }
...
export async function auditLamaloCatalog(): Promise<LamaloCatalogAudit> {
  ...
  if (!canonicalProfile) {
    return {
      ...
      collections: [],
      issues: ["Lamalo Fashions designer profile is missing."],
      auditedAt: new Date().toISOString(),
    };
  }

  const collections = await dbConn
    .select()
    .from(designerCollections)
    .where(eq(designerCollections.designerProfileId, canonicalProfile.id));
  const collectionNameCounts = new Map<string, number>();
  for (const collection of collections) {
    collectionNameCounts.set(collection.name, (collectionNameCounts.get(collection.name) || 0) + 1);
  }
  const duplicateCollectionNames = Array.from(collectionNameCounts.entries())
  ...
  for (const collection of collections) {
    const items = await dbConn
      ...
      .where(and(
        ...
        eq(wardrobeItems.designerProfileId, canonicalProfile.id),
      ));
  ...
  const issues: string[] = [];
  if (collections.length !== EXPECTED_LAMALO_COLLECTION_COUNT) {
    issues.push(`Expected ${EXPECTED_LAMALO_COLLECTION_COUNT} Lamalo collections but found ${collections.length}.`);
  }
...
/**
 * Existing seed logic already knows the complete house catalogue and performs
 * idempotent missing-item/image/price backfills. This function reuses that
 * established implementation, then refuses to report success unless the
 * independent integrity audit passes.
 */
export async function repairAndAuditLamaloCatalog(): Promise<LamaloCatalogAudit> {
  await runLamaloSeed();
  const audit = await auditLamaloCatalog();
  if (!audit.healthy) {
    throw new Error(`Lamalo catalogue repair completed but integrity checks still fail: ${audit.issues.join(" ")}`);
  }
  return audit;
}

export async function getLamaloCatalogSummary(): Promise<{
  collectionCount: number;
  itemCount: number;
  invalidItemCount: number;
  healthy: boolean;
  ...
  return {
    ...
    invalidItemCount: audit.invalidItemCount,
